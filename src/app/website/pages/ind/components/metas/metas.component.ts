...
export class MetasComponent implements OnInit {
  ...
  localidadesList:
    | { label: string; value: { id: number; division: string } }[]
    | undefined;
  ...
  ngOnInit() {
    ...
    this.empresaList?.unshift({
      label: this.sessionService.getEmpresa()?.razonSocial!,
      value: this.sessionService.getEmpresa()?.nit!,
    });
  }
  ...
  onSelectFiltro() {
    if (this.modulo && this.anio && this.paisSelected) {
      this.cargando = true;
      Promise.all([this.getAreas().then(), this.getPlantas().then()])
        .then(() => {
  ...
  async generarFormulario() {
    ...
    try {
      this.metasData = this.areaList?.map((area) => {
        ...
        meta.metas = this.localidadesList
          ?.filter(
            (localidad) =>
              localidad.value.division == String(meta.referencia).toString()
          )
          .map((localidad) => {
            ...
            metaPlanta.valorMeta = Modulos[this.modulo!].map(
              (metaModulo: string) => {
                let valor = {} as ValorMeta;
                valor.referencia = metaModulo;
                valor.value = 0;
  ...
  async cargarDatos() {
    ...
    filterQuery.filterList = [
      ...
      { criteria: Criteria.EQUALS, field: 'pais', value1: this.paisSelected },
      {
        criteria: Criteria.EQUALS,
        field: 'anio',
        value1: this.anio?.getFullYear().toString(),
      },
    ];
  ...
  async getAreas() {
    ...
    if (this.paisSelected == 'eeuu_camex')
      filterQuery.filterList.push({
        criteria: Criteria.EQUALS,
        field: 'nombre',
        value1: 'Bathrooms and Kitchen',
      });
  ...
  async getPlantas() {
    ...
    return new Promise((resolve, reject) => {
      this.plantasService
        .getPlantaWithFilter(filterQuery)
        .then((res: any) => {
          this.localidadesList = (<any[]>res['data']).map((localidad) => {
            return {
              label: localidad.nombre,
              value: { id: localidad.id, division: localidad.area.id },
            };
          });
          resolve(1);
        })
  ...
  getLocalidadNombre(id: number): string {
    return this.localidadesList?.filter(
      (localidad) => localidad.value.id == id
    )[0].label!;
  }

  updateMetaValue(id: any, referencia: any, valueMetaLoc: number) {
    const indice = this.metasData.findIndex((el) => el.referencia == id);
    const indice2 = this.metasData[indice].valorMeta!.findIndex(
      (el) => el.referencia == referencia
    );
        ...
        if (meta.metas.length > 0)
          for (const [j, meta2] of meta.metas.entries()) {
            ...
            if (metaCopySon.length > 0)
              for (const ref of order) {
                let b: any = metaCopySon.find(
                  (resp: any) => resp.referencia == ref
                );
                valorMetaSon.push(b);
              }
