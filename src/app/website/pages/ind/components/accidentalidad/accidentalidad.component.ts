...
export class AccidentalidadComponent
  implements OnInit, AfterViewInit, OnDestroy
{
  ...
  fieldsR: string[] = ['id'];
  fieldsAD: string[] = ['incapacidades', 'fechaElaboracion'];
  fieldsD: string[] = ['analisisId'];
  fieldR: string[] = ['id'];
  ...
  mesesILI2: string[] = [];
  Indicadores: any[] = [
    { label: 'Tasa de Frecuencia', value: 0 },
    { label: 'Tasa de Severidad', value: 1 },
    { label: 'Proporción AT mortal', value: 2 },
  ];
  Eventos: any[] = [
    { label: 'Número total de AT', value: 0 },
    { label: 'Número de AT con días perdidos', value: 4 },
    { label: 'Numero de días perdidos', value: 1 },
    { label: 'Numero de AT mortales', value: 2 },
    { label: 'Numero de AT con cero días', value: 3 },
  ];
  grafMetas: any[] = [
    { label: 'Número de AT con días perdidos', value: 0 },
    { label: 'Meta de AT', value: 1 },
    { label: 'Número de días perdidos', value: 2 },
    { label: 'Meta de días perdidos', value: 2 },
  ];
  grafMetas2: any[] = [
    { label: 'Tasa de Frecuencia', value: 0 },
    { label: 'Meta de TF', value: 1 },
    { label: 'Tasa de Severidad', value: 2 },
    { label: 'Meta de TS', value: 2 },
  ];
  ...
  ngAfterViewInit() {
    this.cargarEventosAt()
      .then(() => {
        ...
        this.selectRangoDiasPerdidosAt(
          this.filtroFechaDiasPerdidos[0],
          'desde'
        );
        // this.getTasas_1();
        // this.getTasas_1_2();
  ...
  async ngOnInit() {
    ...
    if (this.ili <= this.metaIli) {
      ...
      this.colorIli = 'card l-bg-red-dark';
    }
  ...
  async getData() {
    ...
    await this.areaService.findByFilter(areafiltQuery).then((resp: any) => {
      this.areaList = <Area[]>resp['data'];
      let cont = 0;
      this.areaList.forEach((element) => {
        this.divisionesCoronaConId.push({
    ...
    await this.reporteAtService.findAllRAT().then((res) => {
      ...
      this.areaList.forEach((element) => {
        ...
        this.totalDiasPerdidosDv!.push({
          name: element['nombre'],
          value: diasPerdidos,
        });
        this.totalEventosDv!.push({ name: element['nombre'], value: cont });
        this.reporteTabla2.push({
          nombre: element['nombre'],
          eventos: cont,
          dias_Perdidos: diasPerdidos,
        });
        this.totalDiasEventos!.push({
          name: element['nombre'],
          series: [
            { name: 'Eventos AT', value: cont },
            { name: 'Días perdidos', value: diasPerdidos },
          ],
        });
      });
    });
  ...
  async loadResumen() {
    ...
    let hhtEmpresa: Hht[] = [];
    let hhtTemp: Hht[] = [];
    let reportesAt: any[] = JSON.parse(localStorage.getItem('reportesAt')!).map(
      (at: any) => at
    );
    ...
    filterQueryCoronameta.filterList = [
      {
        ...
        field: 'anio',
        value1: this.anioActualResumen.toString(),
      },
    ...
    if (this.selectPais1)
      filterQueryCoronameta.filterList.push({
        criteria: Criteria.EQUALS,
        field: 'pais',
        value1: this.selectPais1.toString(),
      });
    if (this.selectedDivisionResumen1)
      filterQueryCoronameta.filterList.push({
        criteria: Criteria.EQUALS,
        field: 'nombreDivision',
        value1: this.selectedDivisionResumen1.toString(),
      });
    if (this.PlantaSelect1)
      filterQueryCoronameta.filterList.push({
        criteria: Criteria.EQUALS,
        field: 'nombrePlanta',
        value1: this.PlantaSelect1.toString(),
      });
    ...
    if (this.selectPais1)
      if (this.selectPais1 != 'Corona Total')
        filterQueryCorona.filterList.push({
          criteria: Criteria.EQUALS,
          field: 'planta.pais',
          value1: this.selectPais1.toString(),
        });
    if (this.selectedDivisionResumen1)
      filterQueryCorona.filterList.push({
        criteria: Criteria.EQUALS,
        field: 'planta.area.id',
        value1: this.divisionesCoronaConId
          .find((div: any) => div.nombre === this.selectedDivisionResumen1)
          .id.toString(),
      });
    if (this.PlantaSelect1)
      filterQueryCorona.filterList.push({
    ...
    if (this.selectPais1)
      if (this.selectPais1 != 'Corona Total')
        filterQueryTemp.filterList.push({
          criteria: Criteria.EQUALS,
          field: 'planta.pais',
          value1: this.selectPais1.toString(),
        });
    ...
    if (this.fechaFinalResumen) {
      ...
      this.fechaFinalResumen = new Date(
        this.fechaFinalResumen.getFullYear(),
        this.fechaFinalResumen.getMonth() + 1,
        0
      );
    }
    ...
    if (this.selectPais1)
      if (this.selectPais1 != 'Corona Total')
        reportesAt = reportesAt.filter((at) => at.pais == this.selectPais1);
    if (this.selectedDivisionResumen1)
      reportesAt = reportesAt.filter(
        (at) => at.padreNombre == this.selectedDivisionResumen1
      );
    if (this.PlantaSelect1)
      reportesAt = reportesAt.filter(
        (at) => at.nombrePlanta == this.PlantaSelect1
      );
    //fin nuevo

    reportesAt = reportesAt.filter(
      (at) => new Date(at.fechaReporte).getFullYear() == this.anioActualResumen
    );

    reportesAt = reportesAt.filter((at) => {
      return (
        new Date(at['fechaReporte']) >= this.fechaInicioResumen! &&
        new Date(at['fechaReporte']) < this.fechaFinalResumen!
      );
    });

    if (
      this.selectedDivisionResumen &&
      this.selectedDivisionResumen !== 'Total'
    )
      reportesAt = reportesAt.filter(
        (at) => at.padreNombre === this.selectedDivisionResumen
      );
    ...
    reportesAt.forEach((at) => {
      if (at['incapacidades'] != null && at['incapacidades'] != 'null') {
        this.diasPerdidos =
          this.diasPerdidos +
          JSON.parse(at['incapacidades']).reduce(
            (count: any, incapacidad: any) => {
      ...
      .then(async (res: any) => {
        if (res.data.length > 0) {
          ...
          if (this.selectPais1 && !this.selectedDivisionResumen1) {
            ...
            let hhtEmpresaFind: any = hhtmeta.find(
              (ele: any) => ele.nombrePlanta == this.PlantaSelect1
            );
    ...
    await this.hhtService
      .findByFilter(filterQueryCorona)
      .then(async (res: any) => {
        if (res.data.length > 0) {
    ...
    accidentesConDiasPerdidos = reportesAt
      ...
      .filter((at) => {
        let diasTotales = (<Array<any>>JSON.parse(at.incapacidades)).reduce(
          (count, incapacidad) => {
            return count + incapacidad.diasAusencia;
          },
          0
    ...
    totalHhtEmpresa = this.calcularTotalHht(
      hhtEmpresa,
      mesInicio,
      mesFinal,
      this.selectPais1,
    ...
    totalHHtTemporales = this.calcularTotalHht(
      hhtTemp,
      mesInicio,
      mesFinal,
      this.selectPais1,
    ...
    let IF =
      (accidentesConDiasPerdidos / (totalHhtEmpresa + totalHHtTemporales)) *
      240000;
    let IS =
      (totalDiasSeveridad / (totalHhtEmpresa + totalHHtTemporales)) * 240000;
    let ILI = (IF * IS) / 1000;
  ...
  calcularTotalHht(
    ...
    mesFinal: number,
    selectPais: string,
    selectedDivisionResumen: string,
    PlantaSelect: string
  ): number {
    if (selectPais)
      if (selectPais != 'Corona Total')
        hht = hht.filter((eve: any) => eve.planta_pais == selectPais);
    if (selectedDivisionResumen)
      hht = hht.filter(
        (eve: any) =>
          eve.planta_area_id ==
          this.divisionesCoronaConId.find(
            (div: any) => div.nombre === selectedDivisionResumen
          ).id
      );
    if (PlantaSelect)
      hht = hht.filter((eve: any) => eve.planta_nombre == PlantaSelect);
  ...
  async cargarEventosAt(): Promise<boolean> {
    return new Promise((resolve, reject) => {
      this.reporteAtService
        .getAllAt()
        .then((reportesAt: any[]) => {
          localStorage.setItem(
            'reportesAt',
            JSON.stringify(reportesAt.map((at) => at))
          );
          resolve(true);
        })
        .catch((err) => {
  ...
  getEventosAt(filter?: string) {
    if (this.divisionList2.length > 0) {
      ...
      let reporteAt = JSON.parse(localStorage.getItem('reportesAt')!).map(
        (at: any) => at
      );
      ...
      try {
        switch (filter) {
          case 'temp':
            ...
            divisiones.forEach((division) => {
              ...
              auxRandomEv1Dona = auxRandomEv1Dona.concat(
                reporteAt.filter(
                  (at: any) => at.padreNombre === division && at.temporal
                )
              );
              data.value = reporteAt.filter(
                (at: any) => at.padreNombre === division && at.temporal
              ).length;
          ...
          case 'dir':
            ...
            divisiones.forEach((division) => {
              ...
              data.value = reporteAt.filter(
                (at: any) => at.padreNombre === division && at.temporal == null
              ).length;
        ...
        divisiones.forEach((division) => {
          let data = { name: division, value: 0 };
          auxRandomEv1Dona = auxRandomEv1Dona.concat(
            reporteAt.filter((at: any) => at.padreNombre === division)
          );
          data.value = reporteAt.filter(
            (at: any) => at.padreNombre === division
          ).length;
          eventosAt.push(data);
        });
      ...
      localStorage.setItem(
        'reporteAtList',
        JSON.stringify(auxRandomEv1Dona.map((at: any) => at))
      );
    }
  }
  ...
  selectRangoEventosAt(event: Date | any, filter: string) {
    if (this.divisionList2.length > 0) {
      ...
      if (this.filtroFechaAt![0] && this.filtroFechaAt![1]) {
        let dataEv1Dona: any[] = JSON.parse(
          localStorage.getItem('reporteAtList')!
        );
        ...
        let dateFinal: Date = new Date(
          new Date(this.filtroFechaAt![1]).setMonth(
            new Date(this.filtroFechaAt![1]).getMonth() + 1
          )
        );
        dataEv1Dona = dataEv1Dona.filter(
          (at) =>
            at.fechaReporte >= this.filtroFechaAt![0] &&
            at.fechaReporte < dateFinal
        );
        ...
        if (this.selectedDivisionResumen2)
          dataEv1Dona = dataEv1Dona.filter(
            (at) => at.padreNombre == this.selectedDivisionResumen2
          );
        if (this.PlantaSelect2)
          if (this.PlantaSelect2.length > 0) {
            reportesAtCopyDiv = [];
            this.PlantaSelect2.forEach((element: any) => {
              reportesAtCopyDiv = reportesAtCopyDiv.concat(
                dataEv1Dona.filter((at) => at.nombrePlanta == element)
              );
            });
        ...
        divisiones.forEach((division) => {
          let data = { name: division, value: 0 };
          data.value = dataEv1Dona.filter(
            (at) => at.padreNombre === division
          ).length;
  ...
  getDiasPerdidosAt(filter?: string) {
    if (this.divisionList3.length > 0) {
      let divisiones: string[] = [];
      ...
      try {
        switch (filter) {
          case 'temp':
            divisiones.forEach((division) => {
              let data = { name: division, value: 0 };
              auxRandomEv1Donadb = auxRandomEv1Donadb.concat(
                reportesAt.filter(
                  (at) =>
                    at.padreNombre === division &&
              ...
              data.value = reportesAt
                ...
                .reduce((count, itemActual) => {
                  return (
                    count +
                    JSON.parse(itemActual.incapacidades).reduce(
                      (count2: any, dataIncapacidad: any) => {
      ...
      localStorage.setItem(
        'diasPerdidosAtList',
        JSON.stringify(auxRandomEv1Donadb.map((at: any) => at))
      );
    }
  }
  ...
  selectRangoDiasPerdidosAt(event: Date | any, filter: string) {
    if (this.divisionList3.length > 0) {
      if (typeof this.filtroFechaDiasPerdidos === 'undefined')
      ...
      if (this.filtroFechaDiasPerdidos[0] && this.filtroFechaDiasPerdidos[1]) {
        let dataDiasPerdidosAtList: any[] = JSON.parse(
          localStorage.getItem('diasPerdidosAtList')!
        );
        let listaDivisiones: any[] = dataDiasPerdidosAtList.map(
          (at) => at.padreNombre
        );
        let divisiones: any[] = listaDivisiones.filter((item, index) => {
          return listaDivisiones.indexOf(item) === index;
        ...
        if (this.selectPais3)
          if (this.selectPais3 != 'Corona Total')
            dataDiasPerdidosAtList = dataDiasPerdidosAtList.filter(
              (at) => at.pais == this.selectPais3
            );
        if (this.selectedDivisionResumen3)
          dataDiasPerdidosAtList = dataDiasPerdidosAtList.filter(
            (at) => at.padreNombre == this.selectedDivisionResumen3
          );
        if (this.PlantaSelect3)
          if (this.PlantaSelect3.length > 0) {
            reportesAtCopyDiv = [];
            this.PlantaSelect3.forEach((element: any) => {
              reportesAtCopyDiv = reportesAtCopyDiv.concat(
                dataDiasPerdidosAtList.filter(
                  (at) => at.nombrePlanta == element
                )
              );
            });
        ...
        //fin nuevo
        let dateFinal: Date = new Date(
          new Date(this.filtroFechaDiasPerdidos[1]).setMonth(
            new Date(this.filtroFechaDiasPerdidos[1]).getMonth() + 1
          )
        );
        dataDiasPerdidosAtList = dataDiasPerdidosAtList.filter(
          (at) =>
            at.fechaReporte >= this.filtroFechaDiasPerdidos[0] &&
            at.fechaReporte < dateFinal
        );
        let randomEv1Donadb: any[] = [];
        divisiones.forEach((division) => {
          let data = { name: division, value: 0 };
          data.value = dataDiasPerdidosAtList
            .filter(
              (at) =>
                at.padreNombre === division &&
                at.incapacidades != null &&
            ...
            .reduce((count, item) => {
              return (
                count +
                JSON.parse(item.incapacidades).reduce(
                  (count2: any, dataIncapacidad: any) => {
                    return count2 + dataIncapacidad.diasAusencia;
                  },
                  0
  ...
  async getTasas_1(filter?: string) {
    ...
    if (this.plantasList4 || flagCoronaTotal)
      if (
        this.plantasList4.length > 0 ||
        (flagCoronaTotal && this.divisionList4.length > 0)
      ) {
        ...
        if (this.selectedMesesTasa1.length > 0) {
          reportesAt = JSON.parse(localStorage.getItem('reportesAt')!).filter(
            (at: any) => {
              let fechaReporte = new Date(at.fechaReporte);
              if (
                fechaReporte.getFullYear() === this.filtroAnioTasa_1 &&
                this.selectedMesesTasa1.includes(
                  this.Meses[fechaReporte.getMonth()].value
                )
              ) {
                return at;
              }
          ...
          reportesAt = JSON.parse(localStorage.getItem('reportesAt')!).filter(
            (at: any) =>
              new Date(at.fechaReporte).getFullYear() === this.filtroAnioTasa_1
          );
        }
        ...
        if (this.selectPais4)
          if (this.selectPais4 != 'Corona Total')
            reportesAt = reportesAt.filter((at) => at.pais == this.selectPais4);
        if (!flagCoronaTotal)
          if (this.selectedDivisionResumen4)
            reportesAt = reportesAt.filter(
              (at) => at.padreNombre == this.selectedDivisionResumen4
            );
        if (flagCoronaTotal)
          if (this.selectedDivisionResumen4)
            if (this.selectedDivisionResumen4.length > 0) {
              ...
              this.selectedDivisionResumen4.forEach((element: any) => {
                reportesAtCopyDiv = reportesAtCopyDiv.concat(
                  reportesAt.filter((at) => at.padreNombre == element)
                );
              });
        ...
        if (this.PlantaSelect4)
          if (this.PlantaSelect4.length > 0) {
            ...
            this.PlantaSelect4.forEach((element: any) => {
              reportesAtCopyDiv = reportesAtCopyDiv.concat(
                reportesAt.filter((at) => at.nombrePlanta == element.nombre)
              );
            });
            reportesAt = [...reportesAtCopyDiv];
          }
          ...
          filterQuery.filterList = [
            {
              criteria: Criteria.EQUALS,
              field: 'anio',
              value1: this.filtroAnioTasa_1.toString(),
            },
            {
              criteria: Criteria.EQUALS,
              field: 'empresaSelect',
          ...
          if (this.selectPais4)
            if (this.selectPais4 != 'Corona Total')
              filterQuery.filterList.push({
                criteria: Criteria.EQUALS,
                field: 'planta.pais',
                value1: this.selectPais4.toString(),
              });
          if (this.selectedDivisionResumen4)
            if (!flagCoronaTotal)
              filterQuery.filterList.push({
                ...
                value1: this.divisionesCoronaConId
                  .find(
                    (div: any) => div.nombre === this.selectedDivisionResumen4
                  )
                  .id.toString(),
              });
          ...
          await this.hhtService
            ...
            .then(async (res: any) => {
                ...
                if (!flagCoronaTotal)
                  filterQuery2.filterList.push({
                    ...
                    value1: this.divisionesCoronaConId
                      .find(
                        (div: any) =>
                          div.nombre === this.selectedDivisionResumen4
                      )
                      .id.toString(),
                  });

              await this.hhtService
                .findByFilter(filterQuery2)
                .then((res2: any) => {
              ...
              if (res.data.length > 0 || hhtTemp!) {
                ...
                let variableGraf = flagCoronaTotal
                  ? this.divisionesCoronaConId
                  : this.plantasList4;
                variableGraf.forEach((plantas: any) => {
                    ...
                    if (this.selectedMesesTasa1.length > 0) {
                      if (this.selectedMesesTasa1.includes(eleHHt.mes)) {
                        if (
                          plantaL.id ===
                          eleHHt[
                            flagCoronaTotal ? 'planta_area_id' : 'planta_id'
                          ]
                        ) {
                          trabajadoresPorPlanta += eleHHt.numeroPersonas
                            ? eleHHt.numeroPersonas
                            : 0;
                        }
                      ...
                      if (
                        plantaL.id ===
                        eleHHt[flagCoronaTotal ? 'planta_area_id' : 'planta_id']
                      ) {
                        trabajadoresPorPlanta += eleHHt.numeroPersonas
                          ? eleHHt.numeroPersonas
                          : 0;
                  ...
                  if (mesesFiltrados > 0)
                    trabajadoresTotales = trabajadoresTotales / mesesFiltrados;
                  else
                    trabajadoresTotales =
                      trabajadoresTotales /
                      (this.filtroAnioTasa_1 == new Date().getFullYear()
                        ? new Date().getMonth() + 1
                        : 12);
                        ...
                        hhtTemp.forEach((hht: any, indexHHT) => {
                          if (mes === hht.mes) {
                            if (
                              ...
                              let totalTrabajadores = hht.numeroPersonas
                                ? hht.numeroPersonas!
                                : 0;
                  ...
                  if (this.selectedMesesTasa1.length > 0) {
                    totalTrabajadoresTemp = trabajadoresPorMes.reduce(
                      (count: any, trabajadores: any) => {
                    ...
                    totalTrabajadoresTemp =
                      totalTrabajadoresTemp / this.selectedMesesTasa1.length;
                  } else {
                    totalTrabajadoresTemp = trabajadoresPorMes.reduce(
                      (count: any, trabajadores: any) => {
                        return count + trabajadores;
                  ...
                  let totalAt = reportesAt.filter(
                    (at) =>
                      at[flagCoronaTotal ? 'padreNombre' : 'nombrePlanta'] ===
                      plantaL.nombre
                  ).length;
                  let diasPerdidos = reportesAt
                    .filter(
                      (at) =>
                        at[flagCoronaTotal ? 'padreNombre' : 'nombrePlanta'] ===
                          plantaL.nombre &&
                  ...
                  let AtMortales = reportesAt.filter(
                    (at) =>
                      at[flagCoronaTotal ? 'padreNombre' : 'nombrePlanta'] ===
                        plantaL.nombre && at.causoMuerte === true
                  ).length;
                  ...
                  let TF = isNaN(
                    Number(
                      (totalAt * 100) /
                        (trabajadoresTotales + totalTrabajadoresTemp)
                    )
                  )
                    ? 0.0
                    : Number(
                  ...
                  let PAT = isNaN(Number((AtMortales * 100) / totalAt))
                    ? 0.0
                    : Number(Number((AtMortales * 100) / totalAt).toFixed(6));
                  data.series.push({
                ...
                let mesesYear =
                  this.filtroAnioTasa_1 == new Date().getFullYear()
                    ...
                    : 12;
                let numMesesSelect =
                  this.selectedMesesTasa1.length > 0
                    ? this.selectedMesesTasa1.length
                    : mesesYear;
                let totalesTrabajadores =
                  trabajadoresTotalesMes + totalTrabajadoresTempMes;
                ...
                let dataTotal: any = {
                  name: flagCoronaTotal
                    ? 'Corona Total'
                    : this.selectedDivisionResumen4,
                  series: [],
                };
                ...
                dataTotal.series.push({
                  name: 'Tasa de Frecuencia',
                  value:
                    totalesTrabajadores > 0
                      ? Number(
                          (reportesAt.length * 100 * numMesesSelect) /
                            totalesTrabajadores
                        )
                      : 0,
                });
                dataTotal.series.push({
                  ...
                  value:
                    totalesTrabajadores > 0
                      ? Number(
                          (totalDiasPerdidos * 100 * numMesesSelect) /
                            totalesTrabajadores
                        )
                      : 0,
                });
                dataTotal.series.push({
                  ...
                  value:
                    reportesAt.length > 0
                      ? (AtMortalesTotal * 100) / reportesAt.length
                      : 0,
                });
                ...
                localStorage.setItem(
                  'tasaFrecuencia1',
                  JSON.stringify(tasaFrecuencia1.map((item) => item))
                );
                this.filtroTasas1_1();
  ...
  filtroTasas1_1() {
    ...
    let tasaFrecuencia1: any[] = JSON.parse(
      localStorage.getItem('tasaFrecuencia1')!
    );

    if (flagCoronaTotal)
      if (this.selectedDivisionResumen4)
        if (this.selectedDivisionResumen4.length > 0) {
          tasaFrecuencia1 = tasaFrecuencia1.filter((data) =>
            this.selectedDivisionResumen4.includes(data.name)
          );
        }
    ...
    if (!flagCoronaTotal)
      if (this.PlantaSelect4)
        if (this.PlantaSelect4.length > 0) {
          ...
          tasaFrecuencia1 = tasaFrecuencia1.filter((data) =>
            plantas.includes(data.name)
          );
        }

    if (this.selectIndicarores1)
      if (this.selectIndicarores1.length > 0) {
        let indicadores = this.selectIndicarores1.map(
          (indicador) => indicador.label
        );
        tasaFrecuencia1.forEach((tf1) => {
          tf1.series = tf1.series.filter((dataSeries: any) =>
            indicadores.includes(dataSeries.name)
          );
        });
      }
    Object.assign(this, { tasaFrecuencia1 });
  ...
  async getTasas_1_2(filter?: string) {
    ...
    if (this.plantasList4_2 || flagCoronaTotal)
      if (
        this.plantasList4_2.length > 0 ||
        (flagCoronaTotal && this.divisionList4_2.length > 0)
      ) {
        ...
        if (this.selectedMesesTasa1_2.length > 0) {
          reportesAt = JSON.parse(localStorage.getItem('reportesAt')!).filter(
            (at: any) => {
              let fechaReporte = new Date(at.fechaReporte);
              if (
                fechaReporte.getFullYear() === this.filtroAnioTasa_1_2 &&
                this.selectedMesesTasa1_2.includes(
                  this.Meses[fechaReporte.getMonth()].value
                )
              ) {
                return at;
          ...
          reportesAt = JSON.parse(localStorage.getItem('reportesAt')!).filter(
            (at: any) =>
              new Date(at.fechaReporte).getFullYear() ===
              this.filtroAnioTasa_1_2
          );
        }
        ...
        if (!flagCoronaTotal)
          if (this.selectedDivisionResumen4_2)
            reportesAt = reportesAt.filter(
              (at) => at.padreNombre == this.selectedDivisionResumen4_2
            );
        if (flagCoronaTotal)
          if (this.selectedDivisionResumen4_2)
        ...
        if (this.PlantaSelect4_2)
          if (this.PlantaSelect4_2.length > 0) {
            ...
            this.PlantaSelect4_2.forEach((element: any) => {
              reportesAtCopyDiv = reportesAtCopyDiv.concat(
                reportesAt.filter((at) => at.nombrePlanta == element)
              );
            });
            reportesAt = [...reportesAtCopyDiv];
          }
          ...
          filterQuery.filterList = [
            {
              criteria: Criteria.EQUALS,
              field: 'anio',
              value1: this.filtroAnioTasa_1_2.toString(),
            },
            {
              criteria: Criteria.EQUALS,
              field: 'empresaSelect',
          ...
          if (this.selectPais4_2)
            if (this.selectPais4_2 != 'Corona Total')
              filterQuery.filterList.push({
                criteria: Criteria.EQUALS,
                field: 'planta.pais',
                value1: this.selectPais4_2.toString(),
              });
          if (this.selectedDivisionResumen4_2)
            if (!flagCoronaTotal)
              filterQuery.filterList.push({
                ...
                value1: this.divisionesCoronaConId
                  .find(
                    (div: any) => div.nombre === this.selectedDivisionResumen4_2
                  )
                  .id.toString(),
              });
                ...
                if (!flagCoronaTotal)
                  filterQuery2.filterList.push({
                    ...
                    value1: this.divisionesCoronaConId
                      .find(
                        (div: any) =>
                          div.nombre === this.selectedDivisionResumen4_2
                      )
                      .id.toString(),
                  });
              await this.hhtService
              ...
              if (res.data.length > 0 || hhtTemp!) {
                ...
                variableGraf.forEach((plantas: any) => {
                  ...
                  if (mesesFiltrados > 0)
                    ...
                    trabajadoresTotales =
                      trabajadoresTotales /
                      (this.filtroAnioTasa_1_2 == new Date().getFullYear()
                        ? new Date().getMonth() + 1
                        : 12);
                  ...
                  if (this.selectedMesesTasa1_2.length > 0) {
                    ...
                    totalTrabajadoresTempMes += totalTrabajadoresTemp;
                    totalTrabajadoresTemp =
                      totalTrabajadoresTemp / this.selectedMesesTasa1_2.length;
                  } else {
                    totalTrabajadoresTemp = trabajadoresPorMes.reduce(
                      (count: any, trabajadores: any) => {
                        return count + trabajadores;
                ...
                let mesesYear =
                  this.filtroAnioTasa_1_2 == new Date().getFullYear()
                    ...
                    : 12;
                let numMesesSelect =
                  this.selectedMesesTasa1_2.length > 0
                    ? this.selectedMesesTasa1_2.length
                    : mesesYear;
                let totalesTrabajadores =
                  trabajadoresTotalesMes + totalTrabajadoresTempMes;
                ...
                let dataTotal: any = {
                  name: flagCoronaTotal
                    ? 'Corona Total'
                    : this.selectedDivisionResumen4_2,
                  series: [],
                };
                ...
                localStorage.setItem(
                  'tasaFrecuencia1_2',
                  JSON.stringify(tasaFrecuencia1_2.map((item) => item))
                );
  ...
  filtroTasas1_1_2() {
    ...
    let tasaFrecuencia1_2: any[] = JSON.parse(
      localStorage.getItem('tasaFrecuencia1_2')!
    );

    if (flagCoronaTotal)
      if (this.selectedDivisionResumen4_2)
        if (this.selectedDivisionResumen4_2.length > 0) {
          tasaFrecuencia1_2 = tasaFrecuencia1_2.filter((data) =>
            this.selectedDivisionResumen4_2.includes(data.name)
          );
        }
    ...
    if (this.selectIndicarores1_2)
      if (this.selectIndicarores1_2.length > 0) {
        let indicadores = this.selectIndicarores1_2.map(
          (indicador) => indicador.label
        );
  ...
  async getTasas_2(filter?: string) {
    ...
    let reportesAt: any[] = JSON.parse(
      localStorage.getItem('reportesAt')!
    ).filter(
      (at: any) =>
        new Date(at.fechaReporte).getFullYear() === this.filtroAnioTasa_2
    );
    ...
    if (this.selectPais5)
      if (this.selectPais5 != 'Corona Total')
        reportesAt = reportesAt.filter((at) => at.pais == this.selectPais5);
    if (this.selectedDivisionResumen5)
      reportesAt = reportesAt.filter(
        (at) => at.padreNombre == this.selectedDivisionResumen5
      );
      ...
      filterQuery.filterList = [
        {
          criteria: Criteria.EQUALS,
          field: 'anio',
          value1: this.filtroAnioTasa_2.toString(),
        },
        {
          criteria: Criteria.EQUALS,
          field: 'empresaSelect',
      ...
      if (this.selectPais5)
        if (this.selectPais5 != 'Corona Total')
          filterQuery.filterList.push({
            criteria: Criteria.EQUALS,
            field: 'planta.pais',
            value1: this.selectPais5.toString(),
          });
      if (this.selectedDivisionResumen5)
        filterQuery.filterList.push({
          criteria: Criteria.EQUALS,
          field: 'planta.area.id',
          value1: this.divisionesCoronaConId
            .find((div: any) => div.nombre === this.selectedDivisionResumen5)
            .id.toString(),
        });
      ...
      await this.hhtService.findByFilter(filterQuery).then(async (res: any) => {
        ...
        if (res.data.length > 0 || hhtTemp!) {
          this.Meses.forEach((mes, index) => {
                ...
                if (this.PlantaSelect5.length > 0) {
                  this.PlantaSelect5.forEach((pl: any) => {
                    if (elem.mes === mes.label) {
                      if (pl == elem.planta_nombre)
                        trabajadoresPorArea += elem.numeroPersonas
                          ? elem.numeroPersonas
                          : 0;
                    }
                  });
                ...
                if (this.PlantaSelect5.length > 0) {
                  this.PlantaSelect5.forEach((pl: any) => {
                    if (hht.mes === mes.label) {
                      if (pl == hht.planta_nombre)
                        trabajadoresTemPorArea += hht.numeroPersonas
                          ? hht.numeroPersonas
                          : 0;
                    }
                  });
                  ...
                  if (hht.mes == mes.label) {
                    totalTrabajadoresTemp += hht.numeroPersonas
                      ? hht.numeroPersonas
                      : 0;
                  }
                }
            ...
            totalAt = reportesAt.filter(
              (at) => index === new Date(at.fechaReporte).getMonth()
            ).length;

            diasPerdidos = reportesAt
              .filter(
                (at) =>
                  index === new Date(at.fechaReporte).getMonth() &&
                  at.incapacidades !== null &&
                  at.incapacidades !== 'null'
              )
              .reduce((count, item) => {
            ...
            atMortales = reportesAt.filter(
              (at) =>
                index === new Date(at.fechaReporte).getMonth() &&
                at.causoMuerte === true
            ).length;

            let tasaFrecuencia = Number(
              Number(
                (totalAt * 100) / (trabajadoresTotales2 + totalTrabajadoresTemp)
              ).toFixed(6)
            );
            data.series.push({
              name: 'Tasa de Frecuencia',
              value:
                isNaN(tasaFrecuencia) || tasaFrecuencia === Infinity
                  ? 0.0
                  : tasaFrecuencia,
            });
            let tasaSeveridad = Number(
              Number(
                (diasPerdidos * 100) /
                  (trabajadoresTotales2 + totalTrabajadoresTemp)
              ).toFixed(6)
            );
            data.series.push({
              name: 'Tasa de Severidad',
              value:
                isNaN(tasaSeveridad) || tasaSeveridad === Infinity
                  ? 0.0
                  : tasaSeveridad,
            });
            let proporcionAtMortal = Number(
              Number((atMortales * 100) / totalAt).toFixed(6)
            );
            data.series.push({
              name: 'Proporción AT mortal',
              value:
                isNaN(proporcionAtMortal) || proporcionAtMortal === Infinity
                  ? 0.0
                  : proporcionAtMortal,
            });
          ...
          localStorage.setItem(
            'tasaFrecuencia2',
            JSON.stringify(tasaFrecuencia2)
          );
  ...
  filtroTasas_2() {
    ...
    if (this.selectMeses1.length > 0) {
      tasaFrecuencia2 = tasaFrecuencia2.filter((tasaXMes: any) =>
        this.selectMeses1.includes(tasaXMes.name)
      );
    }
    ...
    if (this.selectIndicarores2.length > 0) {
      ...
      tasaFrecuencia2 = tasaFrecuencia2.map((tasaXMes: any) => {
        return {
          name: tasaXMes.name,
          series: tasaXMes.series.filter((el: any) =>
            indicadores.includes(el.name)
          ),
        };
      });
    }
  ...
  async getTasas_2_2(filter?: string) {
    ...
    let reportesAt: any[] = JSON.parse(
      ...
      (at: any) =>
        new Date(at.fechaReporte).getFullYear() === this.filtroAnioTasa_2_2
    );
    ...
    if (this.selectPais5_2)
      if (this.selectPais5_2 != 'Corona Total')
        reportesAt = reportesAt.filter((at) => at.pais == this.selectPais5_2);
    if (this.selectedDivisionResumen5_2)
      reportesAt = reportesAt.filter(
        (at) => at.padreNombre == this.selectedDivisionResumen5_2
      );
      ...
      filterQuery.filterList = [
        {
          criteria: Criteria.EQUALS,
          field: 'anio',
          value1: this.filtroAnioTasa_2_2.toString(),
        },
        {
          criteria: Criteria.EQUALS,
          field: 'empresaSelect',
      ...
      if (this.selectPais5_2)
        if (this.selectPais5_2 != 'Corona Total')
          filterQuery.filterList.push({
            criteria: Criteria.EQUALS,
            field: 'planta.pais',
            value1: this.selectPais5_2.toString(),
          });
      if (this.selectedDivisionResumen5_2)
        filterQuery.filterList.push({
          criteria: Criteria.EQUALS,
          field: 'planta.area.id',
          value1: this.divisionesCoronaConId
            .find((div: any) => div.nombre === this.selectedDivisionResumen5_2)
            .id.toString(),
        });
      ...
      await this.hhtService.findByFilter(filterQuery).then(async (res: any) => {
        ...
        if (this.selectedDivisionResumen5_2)
          filterQuery2.filterList.push({
            ...
            value1: this.divisionesCoronaConId
              .find(
                (div: any) => div.nombre === this.selectedDivisionResumen5_2
              )
              .id.toString(),
          });
        ...
        if (res.data.length > 0 || hhtTemp!) {
          ...
          localStorage.setItem(
            'tasaFrecuencia2_2',
            JSON.stringify(tasaFrecuencia2_2)
          );
  ...
  filtroTasas_2_2() {
    let tasaFrecuencia2_2 = JSON.parse(
      localStorage.getItem('tasaFrecuencia2_2')!
    );
    if (this.selectMeses1_2.length > 0) {
      tasaFrecuencia2_2 = tasaFrecuencia2_2.filter((tasaXMes: any) =>
        this.selectMeses1_2.includes(tasaXMes.name)
      );
    }
    ...
    if (this.plantasList6.length > 0 || flagCoronaTotal) {
      ...
      if (this.selectPais6)
        if (this.selectPais6 != 'Corona Total')
          reportesAt = reportesAt.filter((at) => at.pais == this.selectPais6);
      if (!flagCoronaTotal)
        if (this.selectedDivisionResumen6)
          if (!flagCoronaTotal)
            reportesAt = reportesAt.filter(
              (at) => at.padreNombre == this.selectedDivisionResumen6
            );
      if (flagCoronaTotal)
        if (this.selectedDivisionResumen6)
      ...
      let listaDivisiones = flagCoronaTotal
        ? reportesAt.map((at) => at.padreNombre).filter((at) => at != null)
        : reportesAt.map((at) => at.nombrePlanta).filter((at) => at != null);

      let divisiones = listaDivisiones
        .filter((div, index) => {
      ...
      try {
        ...
        if (this.evento1Desde && this.evento1Hasta) {
          let dateFinal: Date = new Date(
            new Date(this.evento1Hasta).setMonth(
              new Date(this.evento1Hasta).getMonth() + 1
            )
          );
          reportesAt = reportesAt.filter(
            (at) =>
              at.fechaReporte >= this.evento1Desde! &&
              at.fechaReporte < dateFinal
          );
        }
        ...
        divisiones.forEach((division: any) => {
          ...
          let numeroAt: number = reportesAt.filter(
            (at) =>
              at[flagCoronaTotal ? 'padreNombre' : 'nombrePlanta'] === division
          ).length;
          let diasPerdidos: number = reportesAt
            .filter(
              (at) =>
                at[flagCoronaTotal ? 'padreNombre' : 'nombrePlanta'] ===
                  division &&
            ...
            .reduce((count, incapacidades) => {
              return (
                count +
                JSON.parse(incapacidades.incapacidades).reduce(
                  (count2: any, incapacidad: any) => {
                    return count2 + incapacidad.diasAusencia;
                  },
                  0
          ...
          let atMortales: number = reportesAt.filter(
            (at) =>
              at[flagCoronaTotal ? 'padreNombre' : 'nombrePlanta'] ===
                division && at.causoMuerte == true
          ).length;
          let atCeroDias: number = reportesAt.filter((at) => {
            if (
              ...
              let contDiasPerdidos = (<Array<any>>(
                JSON.parse(at.incapacidades)
        ...
        localStorage.setItem(
          'dataEventos1',
          JSON.stringify(dataEventos1.map((data) => data))
        );
    ...
    if (this.selectEventos1)
      if (this.selectEventos1.length > 0) {
        ...
        dataEventos1 = dataEventos1.map((data) => {
          return {
            name: data.name,
            series: data.series.filter((item: any) =>
              eventos.includes(item.name)
            ),
          };
        });
      }
    ...
    if (this.plantasList6_2.length > 0 || flagCoronaTotal) {
      ...
      if (this.selectPais6_2)
        if (this.selectPais6_2 != 'Corona Total')
          reportesAt = reportesAt.filter((at) => at.pais == this.selectPais6_2);
      if (!flagCoronaTotal)
        if (this.selectedDivisionResumen6_2)
          reportesAt = reportesAt.filter(
            (at) => at.padreNombre == this.selectedDivisionResumen6_2
          );
      if (flagCoronaTotal)
        if (this.selectedDivisionResumen6_2)
      ...
      try {
        ...
        if (this.evento1Desde_2 && this.evento1Hasta_2) {
          let dateFinal: Date = new Date(
            new Date(this.evento1Hasta_2).setMonth(
              new Date(this.evento1Hasta_2).getMonth() + 1
            )
          );
          reportesAt = reportesAt.filter(
            (at) =>
              at.fechaReporte >= this.evento1Desde_2! &&
              at.fechaReporte < dateFinal
          );
        }
        ...
        let dataTotal: any = {
          name: flagCoronaTotal
            ? 'Corona Total'
            : this.selectedDivisionResumen6_2,
          series: [],
        };
        ...
        localStorage.setItem(
          'dataEventos1_2',
          JSON.stringify(dataEventos1_2.map((data) => data))
        );
        this.filtroEventos_1();
      }
    }
  ...
  filtroEventos_1_2() {
    ...
    let dataEventos1_2: any[] = JSON.parse(
      localStorage.getItem('dataEventos1_2')!
    );
  ...
  getEventos_2() {
    let reportesAt: any[] = JSON.parse(
      ...
      (at: any) =>
        new Date(at.fechaReporte).getFullYear() === this.filtroAnioEventos2
    );
    ...
    if (this.selectPais7)
      if (this.selectPais7 != 'Corona Total')
        reportesAt = reportesAt.filter((at) => at.pais == this.selectPais7);
    if (this.selectedDivisionResumen7)
      reportesAt = reportesAt.filter(
        (at) => at.padreNombre == this.selectedDivisionResumen7
      );
    if (this.PlantaSelect7)
      if (this.PlantaSelect7.length > 0) {
        reportesAtCopyDiv = [];
    ...
    try {
      ...
      if (this.filtroDivisionEventos2.length > 0)
        reportesAt = reportesAt.filter((at) =>
          this.filtroDivisionEventos2.includes(at.padreNombre)
        );
      this.Meses.forEach((mes, index) => {
        ...
        let numeroAt = reportesAt.filter(
          (at) => new Date(at.fechaReporte).getMonth() === index
        ).length;
        let diasPerdidos = reportesAt
          .filter(
            (at) =>
              new Date(at.fechaReporte).getMonth() === index &&
              at.incapacidades !== null &&
              at.incapacidades !== 'null'
          )
          .reduce((count, at) => {
        ...
        let atMortales = reportesAt.filter(
          (at) =>
            new Date(at.fechaReporte).getMonth() === index && at.causoMuerte
        ).length;
      ...
      localStorage.setItem(
        'dataEventos2',
        JSON.stringify(dataEventos2.map((data) => data))
      );
    ...
    if (this.selectMeses2)
      if (this.selectMeses2.length > 0) {
        dataEventos2 = dataEventos2.filter((data) =>
          this.selectMeses2.includes(data.name)
        );
      }
  ...
  getEventos_2_2() {
    let reportesAt: any[] = JSON.parse(
      ...
      (at: any) =>
        new Date(at.fechaReporte).getFullYear() === this.filtroAnioEventos2_2
    );
    let dataEventos2_2: any[] = [];
    //nuevo
    let reportesAtCopyDiv: any[] = [];
    if (this.selectPais7_2)
      if (this.selectPais7_2 != 'Corona Total')
        reportesAt = reportesAt.filter((at) => at.pais == this.selectPais7_2);
    if (this.selectedDivisionResumen7_2)
      reportesAt = reportesAt.filter(
        (at) => at.padreNombre == this.selectedDivisionResumen7_2
      );
    if (this.PlantaSelect7_2)
      if (this.PlantaSelect7_2.length > 0) {
        reportesAtCopyDiv = [];
    ...
    try {
      ...
      if (this.filtroDivisionEventos2_2.length > 0)
        reportesAt = reportesAt.filter((at) =>
          this.filtroDivisionEventos2_2.includes(at.padreNombre)
        );
      this.Meses.forEach((mes, index) => {
        // if(this.Meses.length === index + 1) return;
        let data: any = {
      ...
      localStorage.setItem(
        'dataEventos2_2',
        JSON.stringify(dataEventos2_2.map((data) => data))
      );
  ...
  filtroEventos_2_2() {
    let dataEventos2_2: any[] = JSON.parse(
      localStorage.getItem('dataEventos2_2')!
    );

    if (this.selectMeses2_2)
      if (this.selectMeses2_2.length > 0) {
        dataEventos2_2 = dataEventos2_2.filter((data) =>
          this.selectMeses2_2.includes(data.name)
        );
      }
  ...
  async getIli_1() {
    ...
    let reportesAt: any[] = JSON.parse(
      ...
      (at: any) =>
        new Date(at.fechaReporte).getFullYear() === this.selectedAnioIli_1
    );
    ...
    if (this.selectPais8)
      if (this.selectPais8 != 'Corona Total')
        reportesAt = reportesAt.filter((at) => at.pais == this.selectPais8);
    if (this.selectedDivisionResumen8)
      reportesAt = reportesAt.filter(
        (at) => at.padreNombre == this.selectedDivisionResumen8
      );
    ...
    filterQuery.filterList = [
      {
        criteria: Criteria.EQUALS,
        field: 'anio',
        value1: this.selectedAnioIli_1.toString(),
      },
      {
        criteria: Criteria.EQUALS,
        field: 'empresaSelect',
    ...
    if (this.selectPais8)
      if (this.selectPais8 != 'Corona Total')
        filterQuery.filterList.push({
          criteria: Criteria.EQUALS,
          field: 'planta.pais',
          value1: this.selectPais8.toString(),
        });
    if (this.selectedDivisionResumen8)
      filterQuery.filterList.push({
        criteria: Criteria.EQUALS,
        field: 'planta.area.nombre',
        value1: this.selectedDivisionResumen8,
      });

    await this.hhtService.findByFilter(filterQuery).then(async (res: any) => {
      if (this.filtroMesesIli_1.length > 0)
        reportesAt = reportesAt.filter((at) =>
          this.filtroMesesIli_1.includes(
            this.meses[new Date(at.fechaReporte).getMonth()]
          )
        );
      ...
      filterQueryMeta.filterList = [
        ...
        },
        {
          criteria: Criteria.EQUALS,
          field: 'modulo',
          value1: 'Accidentalidad',
        },
      ];

      if (this.selectPais8)
        filterQueryMeta.filterList.push({
          criteria: Criteria.EQUALS,
          field: 'pais',
          value1: this.selectPais8.toString(),
        });
      ...
      divisionesList.forEach((planta: any, index: any) => {
          ...
          if (this.filtroMesesIli_1.length > 0) {
            if (this.filtroMesesIli_1.includes(elem.mes)) {
              if (elem.planta_nombre == planta) {
                let meta1: viewHHtMetas = meta.find(
                  (met: any) => met.nombrePlanta == planta
                );
        ...
        let totalDiasSeveridad = reportesAt
          .filter(
            (at) =>
              at.nombrePlanta === planta &&
              at.incapacidades !== 'null' &&
        ...
        dataIli_1.datasets[1].data.push(
          isNaN(ILI) ? 0.0 : ILI === Infinity ? 0.0 : Number(ILI.toFixed(6))
        );
        dataIli_1.datasets[0].data.push(metaCorona);
      });

      let IF =
        (accidentesConDiasPerdidosTotal / (hhtTotalEmpresa + hhtTotalTemp)) *
        240000;
      let IS =
        (diasSeveridadTotalEmp / (hhtTotalEmpresa + hhtTotalTemp)) * 240000;
      let ILI = (IF * IS) / 1000;
      ...
      if (meta) {
        if (meta.length > 0)
          metaTotal = meta.find(
            (met: any) => met.nombreDivision == this.selectedDivisionResumen8
          ).iliAnual
            ? meta.find(
                (met: any) =>
                  met.nombreDivision == this.selectedDivisionResumen8
              ).iliAnual
            : 0;
      }
  ...
  filtroIli_1() {
    ...
    let plantasCoronaIli1 = this.plantasList8.map((div: any) => {
      return div.label;
    });
    ...
    if (this.PlantaSelect8)
      if (this.PlantaSelect8.length > 0) {
        ...
        plantasCoronaIli1 = plantasCoronaIli1.filter((div: any) =>
          PlantaSelect.includes(div)
        );
        dataIli_1.labels = plantasCoronaIli1;
        dataIli_1.datasets[1].data = dataIli_1.datasets[1].data.filter(
          (data: any, index: any) =>
            PlantaSelect.includes(plantasCoronaIli1[index])
        );
        dataIli_1.datasets[0].data = dataIli_1.datasets[0].data.filter(
          (data: any, index: any) =>
            PlantaSelect.includes(plantasCoronaIli1[index])
        );
      }
    ...
    await this.hhtService.findByFilter(filterQuery).then(async (res: any) => {
      ...
      this.divisionesCoronaConId.forEach((division, index) => {
        ...
        let accidentesConDiasPerdidos = reportesAt
          .filter(
            (at) =>
              at.padreNombre === division.nombre &&
              at.incapacidades !== null &&
              at.incapacidades !== 'null'
          )
          .filter((at) => {
          ...
          if (this.filtroMesesIli_1.length > 0) {
            if (this.filtroMesesIli_1.includes(elem.mes)) {
              if (elem.planta_area_nombre == division.nombre) {
                let meta1: viewHHtMetas = meta.find(
                  (met: any) => met.nombreDivision == division.nombre
                );
      ...
      if (meta) {
        if (meta.length > 0)
          metaTotal = meta[0].iliAnual ? meta[0].iliAnual : 0;
      }
  ...
  filtroIli_1Total() {
    ...
    if (this.selectedDivisionResumen8)
      if (this.selectedDivisionResumen8.length > 0) {
        ...
        divisionesCoronaIli1 = divisionesCoronaIli1.filter((div) =>
          this.selectedDivisionResumen8.includes(div)
        );
        dataIli_1.labels = divisionesCoronaIli1;
        dataIli_1.datasets[1].data = dataIli_1.datasets[1].data.filter(
          (data: any, index: any) =>
            this.selectedDivisionResumen8.includes(this.divisionesCorona[index])
        );
        dataIli_1.datasets[0].data = dataIli_1.datasets[0].data.filter(
          (data: any, index: any) =>
            this.selectedDivisionResumen8.includes(this.divisionesCorona[index])
  ...
  async getIli_1_2() {
    ...
    let reportesAt: any[] = JSON.parse(
      ...
      (at: any) =>
        new Date(at.fechaReporte).getFullYear() === this.selectedAnioIli_1_2
    );
    ...
    if (this.selectPais8_2)
      if (this.selectPais8_2 != 'Corona Total')
        reportesAt = reportesAt.filter((at) => at.pais == this.selectPais8_2);
    if (this.selectedDivisionResumen8_2)
      reportesAt = reportesAt.filter(
        (at) => at.padreNombre == this.selectedDivisionResumen8_2
      );
    ...
    filterQuery.filterList = [
      {
        criteria: Criteria.EQUALS,
        field: 'anio',
        value1: this.selectedAnioIli_1_2.toString(),
      },
      {
        criteria: Criteria.EQUALS,
        field: 'empresaSelect',
    ...
    if (this.selectPais8_2)
      if (this.selectPais8_2 != 'Corona Total')
        filterQuery.filterList.push({
          criteria: Criteria.EQUALS,
          field: 'planta.pais',
          value1: this.selectPais8_2.toString(),
        });
    if (this.selectedDivisionResumen8_2)
      filterQuery.filterList.push({
        criteria: Criteria.EQUALS,
        field: 'planta.area.nombre',
        value1: this.selectedDivisionResumen8_2,
      });

    await this.hhtService.findByFilter(filterQuery).then(async (res: any) => {
      if (this.filtroMesesIli_1_2.length > 0)
        reportesAt = reportesAt.filter((at) =>
          this.filtroMesesIli_1_2.includes(
            this.meses[new Date(at.fechaReporte).getMonth()]
          )
        );
      ...
      divisionList.forEach((planta: any, index: any) => {
        ...
        dataIli_1_2.datasets[1].data.push(
          isNaN(ILI) ? 0.0 : ILI === Infinity ? 0.0 : Number(ILI.toFixed(6))
        );
      ...
      if (meta) {
        if (meta.length > 0)
          metaTotal = meta.find(
            (met: any) => met.nombreDivision == this.selectedDivisionResumen8_2
          ).iliAnual
            ? meta.find(
                (met: any) =>
                  met.nombreDivision == this.selectedDivisionResumen8_2
              ).iliAnual
            : 0;
      }
  ...
  filtroIli_1_2() {
    ...
    if (this.PlantaSelect8_2)
      if (this.PlantaSelect8_2.length > 0) {
        ...
        dataIli_1_2.datasets[1].data = dataIli_1_2.datasets[1].data.filter(
          (data: any, index: any) =>
            PlantaSelect.includes(plantasCoronaIli1[index])
        );
        dataIli_1_2.datasets[0].data = dataIli_1_2.datasets[0].data.filter(
          (data: any, index: any) =>
            PlantaSelect.includes(plantasCoronaIli1[index])
        );
      }
  ...
  filtroIli_1Total_2() {
    ...
    if (this.selectedDivisionResumen8_2)
      if (this.selectedDivisionResumen8_2.length > 0) {
        // let selectedDivisiones = this.selectedDivisionResumen8
        divisionesCoronaIli1 = divisionesCoronaIli1.filter((div) =>
          this.selectedDivisionResumen8_2.includes(div)
        );
        dataIli_1_2.labels = divisionesCoronaIli1;
        dataIli_1_2.datasets[1].data = dataIli_1_2.datasets[1].data.filter(
          (data: any, index: any) =>
            this.selectedDivisionResumen8_2.includes(
              this.divisionesCorona[index]
            )
        );
        dataIli_1_2.datasets[0].data = dataIli_1_2.datasets[0].data.filter(
          (data: any, index: any) =>
  ...
  async getIli_2(event?: any) {
    ...
    let reportesAt: any[] = JSON.parse(
      ...
      (at: any) =>
        new Date(at.fechaReporte).getFullYear() === this.selectedAnioIli_2
    );
    ...
    if (this.selectPais9)
      if (this.selectPais9 != 'Corona Total')
        reportesAt = reportesAt.filter((at) => at.pais == this.selectPais9);
    if (this.selectedDivisionResumen9)
      reportesAt = reportesAt.filter(
        (at) => at.padreNombre == this.selectedDivisionResumen9
      );
    if (this.PlantaSelect9)
      reportesAt = reportesAt.filter(
        (at) => at.nombrePlanta == this.PlantaSelect9
      );
    //fin nuevo
    ...
    filterQuery.filterList = [
      {
        criteria: Criteria.EQUALS,
        field: 'anio',
        value1: this.selectedAnioIli_2.toString(),
      },
      {
        criteria: Criteria.EQUALS,
        field: 'empresaSelect',
    ...
    if (this.selectPais9)
      if (this.selectPais9 != 'Corona Total')
        filterQuery.filterList.push({
          criteria: Criteria.EQUALS,
          field: 'planta.pais',
          value1: this.selectPais9.toString(),
        });
    if (this.selectedDivisionResumen9)
      filterQuery.filterList.push({
        criteria: Criteria.EQUALS,
        field: 'planta.area.nombre',
        value1: this.selectedDivisionResumen9,
      });
    ...
    await this.hhtService.findByFilter(filterQuery).then(async (res: any) => {
      ...
      if (this.selectedDivisionResumen9)
        filterQueryMeta.filterList.push({
          criteria: Criteria.EQUALS,
          field: 'nombreDivision',
          value1: this.selectedDivisionResumen9.toString(),
        });
      let meta: any;
      filterQueryMeta.filterList.push({
        criteria: Criteria.EQUALS,
      ...
      this.meses.forEach((mes, index) => {
          ...
          if (this.PlantaSelect9) {
            if (mes == elem.mes) {
              ...
              if (this.PlantaSelect9 === elem.planta_nombre) {
                let meta1: viewHHtMetas = meta.find(
                  (met: any) => met.nombrePlanta == this.PlantaSelect9
                );
                metaCorona = Number(meta1.iliPlanta ? meta1.iliPlanta : 0);
                hhtCorona += Number(elem.hht ? elem.hht : 0);
              }
        ...
        let ILIAux = isNaN(ILI)
          ? 0.0
          : ILI === Infinity
          ? 0.0
          : Number(ILI.toFixed(6));
  ...
  filtroIli_2() {
    ...
    if (this.selectMesesILI2.length > 0) {
      ...
      dataIli_2.labels = this.meses.filter((mes) =>
        this.selectMesesILI2.includes(mes)
      );
      dataIli_2.datasets[1].data = dataIli_2.datasets[1].data.filter(
        (data: any, index: any) =>
          this.selectMesesILI2.includes(this.meses[index])
      );
      dataIli_2.datasets[0].data = dataIli_2.datasets[0].data.filter(
        (data: any, index: any) =>
          this.selectMesesILI2.includes(this.meses[index])
      );
    }
  ...
  async getIli_2_2(event?: any) {
    ...
    let reportesAt: any[] = JSON.parse(
      ...
      (at: any) =>
        new Date(at.fechaReporte).getFullYear() === this.selectedAnioIli_2_2
    );
    ...
    if (this.selectPais9_2)
      if (this.selectPais9_2 != 'Corona Total')
        reportesAt = reportesAt.filter((at) => at.pais == this.selectPais9_2);
    if (this.selectedDivisionResumen9_2)
      reportesAt = reportesAt.filter(
        (at) => at.padreNombre == this.selectedDivisionResumen9_2
      );
    if (this.PlantaSelect9_2)
      reportesAt = reportesAt.filter(
        (at) => at.nombrePlanta == this.PlantaSelect9_2
      );
    //fin nuevo
    let dataIli_2_2: {
      labels: any;
    ...
    filterQuery.filterList = [
      {
        criteria: Criteria.EQUALS,
        field: 'anio',
        value1: this.selectedAnioIli_2_2.toString(),
      },
      {
        criteria: Criteria.EQUALS,
        field: 'empresaSelect',
    ...
    if (this.selectPais9_2)
      if (this.selectPais9_2 != 'Corona Total')
        filterQuery.filterList.push({
          criteria: Criteria.EQUALS,
          field: 'planta.pais',
          value1: this.selectPais9_2.toString(),
        });
    if (this.selectedDivisionResumen9_2)
      filterQuery.filterList.push({
        criteria: Criteria.EQUALS,
        field: 'planta.area.nombre',
        value1: this.selectedDivisionResumen9_2,
      });
    ...
    await this.hhtService.findByFilter(filterQuery).then(async (res: any) => {
      ...
      if (this.selectedDivisionResumen9_2)
        filterQueryMeta.filterList.push({
          criteria: Criteria.EQUALS,
          field: 'nombreDivision',
          value1: this.selectedDivisionResumen9_2.toString(),
        });
          ...
          if (this.PlantaSelect9_2) {
            if (mes == elem.mes) {
              ...
              if (this.PlantaSelect9_2 === elem.planta_nombre) {
                let meta1: viewHHtMetas = meta.find(
                  (met: any) => met.nombrePlanta == this.PlantaSelect9_2
                );
                metaCorona = Number(meta1.iliPlanta ? meta1.iliPlanta : 0);
                hhtCorona += Number(elem.hht ? elem.hht : 0);
              }
  ...
  filtroIli_2_2() {
    ...
    if (this.selectMesesILI2_2.length > 0) {
      // mesesILI2 = this.meses.filter(mes => this.selectMesesILI2.includes(mes));
      dataIli_2_2.labels = this.meses.filter((mes) =>
        this.selectMesesILI2_2.includes(mes)
      );
      dataIli_2_2.datasets[1].data = dataIli_2_2.datasets[1].data.filter(
        (data: any, index: any) =>
          this.selectMesesILI2_2.includes(this.meses[index])
      );
      dataIli_2_2.datasets[0].data = dataIli_2_2.datasets[0].data.filter(
        (data: any, index: any) =>
          this.selectMesesILI2_2.includes(this.meses[index])
      );
    }
  ...
  async getMeta_1Eve() {
    ...
    if (this.plantasList10.length > 0 || flagCoronaTotal) {
      let reportesAt: any[] = JSON.parse(
        ...
        (at: any) =>
          new Date(at.fechaReporte).getFullYear() === this.selectedAnioMeta_1
      );
      ...
      if (this.selectPais10)
        if (this.selectPais10 != 'Corona Total')
          reportesAt = reportesAt.filter((at) => at.pais == this.selectPais10);
      if (!flagCoronaTotal)
        if (this.selectedDivisionResumen10)
          if (this.selectPais10 != 'Corona Total')
            reportesAt = reportesAt.filter(
              (at) => at.padreNombre == this.selectedDivisionResumen10
            );
      if (flagCoronaTotal)
        if (this.selectedDivisionResumen10)
      ...
      // let listaDivisiones = reportesAt.map(at => at.padreNombre);
      // let listaDivisiones = reportesAt.map(at => at.nombrePlanta);
      let listaPlantas = flagCoronaTotal
        ? reportesAt.map((at) => at.padreNombre).filter((at) => at != null)
        : reportesAt.map((at) => at.nombrePlanta).filter((at) => at != null);

      // let listaPlantas = reportesAt.map(at => at.nombrePlanta);

      let plantas = listaPlantas
        .filter((div, index) => {
          return listaPlantas.indexOf(div) === index;
        })
        .sort();
      plantas.push(
        flagCoronaTotal ? 'Corona total' : this.selectedDivisionResumen10
      );
      ...
      filterQueryMeta.filterList = [
        {
          criteria: Criteria.EQUALS,
          field: 'anio',
          value1: this.selectedAnioMeta_1.toString(),
        },
        {
          criteria: Criteria.EQUALS,
          field: 'empresaId',
      ...
      if (this.selectPais10)
        filterQueryMeta.filterList.push({
          criteria: Criteria.EQUALS,
          field: 'pais',
          value1: this.selectPais10.toString(),
        });
      if (this.selectedDivisionResumen10)
        if (this.selectPais10 != 'Corona Total')
          filterQueryMeta.filterList.push({
            criteria: Criteria.EQUALS,
            field: 'nombreDivision',
            value1: this.selectedDivisionResumen10,
          });
      ...
      let metaTotal: any;
      filterQueryTotal.filterList.push({
        criteria: Criteria.EQUALS,
        field: 'modulo',
        value1: 'Accidentalidad',
      });
      await this.viewHHtMetasService
        .getWithFilter(filterQueryTotal)
        .then((metas: any) => {
      ...
      try {
        ...
        if (this.filtroMesesMeta_1.length > 0)
          reportesAt = reportesAt.filter((at) =>
            this.filtroMesesMeta_1.includes(
              this.meses[new Date(at.fechaReporte).getMonth()]
            )
          );
        ...
        plantas1.forEach((division: any) => {
          meta1 = flagCoronaTotal
            ? metaTotal.find((met: any) => met.nombreDivision == division)
            : meta.find((met: any) => met.nombrePlanta == division);
          ...
          dataMetaNumATConDiasPerdidosTotal.push(
            meta1
              ? meta1[flagCoronaTotal ? 'eveDivision' : 'evePlanta']
                ? meta1[flagCoronaTotal ? 'eveDivision' : 'evePlanta']
                : 0
              : 0
          );
          dataMetaDiasPerdidosTotal.push(
            meta1
              ? meta1[
                  flagCoronaTotal
                    ? 'diasPerdidosDivision'
                    : 'diasPerdidosPlanta'
                ]
                ? meta1[
                    flagCoronaTotal
        ...
        dataMetaNumATConDiasPerdidosTotal.push(
          meta1
            ? meta1[flagCoronaTotal ? 'eveAnual' : 'eveDivision']
              ? meta1[flagCoronaTotal ? 'eveAnual' : 'eveDivision']
              : 0
            : 0
        );
        dataMetaDiasPerdidosTotal.push(
          meta1
            ? meta1[flagCoronaTotal ? 'dperdidosAnual' : 'diasPerdidosDivision']
              ? meta1[
                  flagCoronaTotal ? 'dperdidosAnual' : 'diasPerdidosDivision'
                ]
              : 0
            : 0
        );
  ...
  filtroMeta_1() {
    ...
    if (this.selectPais10 == 'Corona Total')
      if (this.selectedDivisionResumen10)
        if (this.selectedDivisionResumen10.length > 0) {
          let plantasCoronaMeta10 = this.divisionList10.map(
            (div: any) => div.label
          );

          let PlantaSelect = this.selectedDivisionResumen10
            .map((div: any) => div)
            .sort();

          dataMeta_1.labels = PlantaSelect;

          dataMeta_1.datasets[3].data = dataMeta_1.datasets[3].data.filter(
            (data: any, index: any) =>
              PlantaSelect.includes(plantasCoronaMeta10[index])
          );
          dataMeta_1.datasets[2].data = dataMeta_1.datasets[2].data.filter(
            (data: any, index: any) =>
              PlantaSelect.includes(plantasCoronaMeta10[index])
          );
          dataMeta_1.datasets[1].data = dataMeta_1.datasets[1].data.filter(
            (data: any, index: any) =>
              PlantaSelect.includes(plantasCoronaMeta10[index])
          );
          dataMeta_1.datasets[0].data = dataMeta_1.datasets[0].data.filter(
            (data: any, index: any) =>
              PlantaSelect.includes(plantasCoronaMeta10[index])
          );
        }
    ...
    if (this.selectPais10 != 'Corona Total')
      if (this.PlantaSelect10)
        if (this.PlantaSelect10.length > 0) {
          let plantasCoronaMeta10 = this.plantasList10.map(
            (div: any) => div.label
          );
    ...
    if (this.selectEventos10)
      if (this.selectEventos10.length > 0) {
        ...
        dataMeta_1.datasets = dataMeta_1.datasets.filter(
          (data: any, index: any) =>
            filtroCoronaMeta10.includes(filtrosListCoronaMeta10[index])
        );
      }
  ...
  async getMeta_1Eve_2() {
    ...
    if (this.plantasList10_2.length > 0 || flagCoronaTotal) {
      let reportesAt: any[] = JSON.parse(
        ...
        (at: any) =>
          new Date(at.fechaReporte).getFullYear() === this.selectedAnioMeta_1_2
      );
      ...
      if (!flagCoronaTotal)
        if (this.selectedDivisionResumen10_2)
          reportesAt = reportesAt.filter(
            (at) => at.padreNombre == this.selectedDivisionResumen10_2
          );
      if (flagCoronaTotal)
        if (this.selectedDivisionResumen10_2)
      ...
      let plantas = listaPlantas
        ...
        .sort();
      plantas.push(
        flagCoronaTotal ? 'Corona total' : this.selectedDivisionResumen10_2
      );
      ...
      filterQueryMeta.filterList = [
        {
          criteria: Criteria.EQUALS,
          field: 'anio',
          value1: this.selectedAnioMeta_1_2.toString(),
        },
        {
          criteria: Criteria.EQUALS,
          field: 'empresaId',
      ...
      if (this.selectPais10_2)
        filterQueryMeta.filterList.push({
          criteria: Criteria.EQUALS,
          field: 'pais',
          value1: this.selectPais10_2.toString(),
        });
      if (this.selectedDivisionResumen10_2)
        if (this.selectPais10_2 != 'Corona Total')
          filterQueryMeta.filterList.push({
            criteria: Criteria.EQUALS,
            field: 'nombreDivision',
            value1: this.selectedDivisionResumen10_2,
          });
      ...
      try {
        ...
        if (this.filtroMesesMeta_1_2.length > 0)
          reportesAt = reportesAt.filter((at) =>
            this.filtroMesesMeta_1_2.includes(
              this.meses[new Date(at.fechaReporte).getMonth()]
            )
          );
  ...
  filtroMeta_1_2() {
    ...
    if (this.selectPais10_2 == 'Corona Total')
      if (this.selectedDivisionResumen10_2)
        if (this.selectedDivisionResumen10_2.length > 0) {
          let plantasCoronaMeta10 = this.divisionList10_2.map(
            (div: any) => div.label
          );

          let PlantaSelect = this.selectedDivisionResumen10_2
            .map((div: any) => div)
            .sort();
          dataMeta_1_2.labels = PlantaSelect;

          dataMeta_1_2.datasets[3].data = dataMeta_1_2.datasets[3].data.filter(
            (data: any, index: any) =>
              PlantaSelect.includes(plantasCoronaMeta10[index])
          );
          dataMeta_1_2.datasets[2].data = dataMeta_1_2.datasets[2].data.filter(
            (data: any, index: any) =>
              PlantaSelect.includes(plantasCoronaMeta10[index])
          );
          dataMeta_1_2.datasets[1].data = dataMeta_1_2.datasets[1].data.filter(
            (data: any, index: any) =>
              PlantaSelect.includes(plantasCoronaMeta10[index])
          );
          dataMeta_1_2.datasets[0].data = dataMeta_1_2.datasets[0].data.filter(
            (data: any, index: any) =>
              PlantaSelect.includes(plantasCoronaMeta10[index])
          );
        }
    ...
    if (this.selectPais10_2 != 'Corona Total')
      if (this.PlantaSelect10_2)
        if (this.PlantaSelect10_2.length > 0) {
          let plantasCoronaMeta10 = this.plantasList10_2.map(
            (div: any) => div.label
          );

          let PlantaSelect = this.PlantaSelect10_2.map(
            (div: any) => div
          ).sort();
    ...
    if (this.selectEventos10_2)
      if (this.selectEventos10_2.length > 0) {
        ...
        dataMeta_1_2.datasets = dataMeta_1_2.datasets.filter(
          (data: any, index: any) =>
            filtroCoronaMeta10.includes(filtrosListCoronaMeta10[index])
        );
      }
    ...
    if (this.selectPais12) {
      ...
      if (this.selectPais12)
        if (this.selectPais12 != 'Corona Total')
          reportesAt = reportesAt.filter((at) => at.pais == this.selectPais12);
      if (this.selectedDivisionResumen12)
        reportesAt = reportesAt.filter(
          (at) => at.padreNombre == this.selectedDivisionResumen12
        );
      if (this.PlantaSelect12)
        if (this.PlantaSelect12.length > 0) {
          reportesAtCopyDiv = [];
      ...
      let meses: any[] = [];
      if (this.filtroMesesMeta_1meses.length > 0)
        meses = this.MesesValue.filter((data) =>
          this.filtroMesesMeta_1meses.includes(data.label)
        );
      ...
      try {
        ...
        filterQueryMeta.filterList = [
          {
            criteria: Criteria.EQUALS,
            field: 'anio',
            value1: this.selectedAnioMeta_1meses.toString(),
          },
          {
            criteria: Criteria.EQUALS,
            field: 'empresaId',
        ...
        if (this.selectPais12)
          filterQueryMeta.filterList.push({
            criteria: Criteria.EQUALS,
            field: 'pais',
            value1: this.selectPais12.toString(),
          });
        if (this.selectedDivisionResumen12)
          filterQueryMeta.filterList.push({
            criteria: Criteria.EQUALS,
            field: 'nombreDivision',
            value1: this.selectedDivisionResumen12,
          });
        ...
        let meta1: any = meta.find(
          (met: any) => met.nombreDivision == this.selectedDivisionResumen12
        );

        meses.forEach((mes, index) => {
          let numeroAt = reportesAt.filter(
            (at) => new Date(at.fechaReporte).getMonth() === mes.value
          ).length;
          let diasPerdidos = reportesAt
            .filter(
              (at) =>
                new Date(at.fechaReporte).getMonth() === mes.value &&
                at.incapacidades !== null &&
                at.incapacidades !== 'null'
            )
            .reduce((count, at) => {
          ...
          dataDiasPerdidosTotal.push(diasPerdidos);
          dataMetaNumATConDiasPerdidosTotal.push(
            meta1 ? (meta1.eveDivision ? meta1.eveDivision : 0) : 0
          );
          dataMetaDiasPerdidosTotal.push(
            meta1
              ? meta1.diasPerdidosDivision
                ? meta1.diasPerdidosDivision
                : 0
              : 0
          );
        ...
        localStorage.setItem(
          'dataMeta_1meses',
          JSON.stringify(dataMeta_1meses)
        );
  ...
  filtroEventosMeta_2() {
    let dataMeta_1meses: any = JSON.parse(
      localStorage.getItem('dataMeta_1meses')!
    );
    ...
    if (this.selectEventos12)
      if (this.selectEventos12.length > 0) {
        ...
        dataMeta_1meses.datasets = dataMeta_1meses.datasets.filter(
          (data: any, index: any) =>
            filtroCoronaMeta12.includes(filtrosListCoronaMeta12[index])
        );
      }
  ...
  async getEventosMeta_2_2() {
    if (this.selectPais12_2) {
      let reportesAt: any[] = JSON.parse(
        ...
        (at: any) =>
          new Date(at.fechaReporte).getFullYear() ===
          this.selectedAnioMeta_1meses_2
      );
      // let dataEventos2: any[] = [];
      //nuevo
      let reportesAtCopyDiv: any[] = [];
      ...
      if (this.selectedDivisionResumen12_2)
        reportesAt = reportesAt.filter(
          (at) => at.padreNombre == this.selectedDivisionResumen12_2
        );
      if (this.PlantaSelect12_2)
        if (this.PlantaSelect12_2.length > 0) {
          reportesAtCopyDiv = [];
      ...
      let meses: any[] = [];
      if (this.filtroMesesMeta_1meses_2.length > 0)
        meses = this.MesesValue.filter((data) =>
          this.filtroMesesMeta_1meses_2.includes(data.label)
        );
      else meses = this.MesesValue.map((ele) => ele);
      ...
      try {
        ...
        filterQueryMeta.filterList = [
          {
            criteria: Criteria.EQUALS,
            field: 'anio',
            value1: this.selectedAnioMeta_1meses_2.toString(),
          },
          {
            criteria: Criteria.EQUALS,
            field: 'empresaId',
        ...
        if (this.selectPais12_2)
          filterQueryMeta.filterList.push({
            criteria: Criteria.EQUALS,
            field: 'pais',
            value1: this.selectPais12_2.toString(),
          });
        if (this.selectedDivisionResumen12_2)
          filterQueryMeta.filterList.push({
            criteria: Criteria.EQUALS,
            field: 'nombreDivision',
            value1: this.selectedDivisionResumen12_2,
          });
        ...
        let meta1: any = meta.find(
          (met: any) => met.nombreDivision == this.selectedDivisionResumen12_2
        );
        ...
        localStorage.setItem(
          'dataMeta_1meses_2',
          JSON.stringify(dataMeta_1meses_2)
        );
        this.filtroEventosMeta_2();
      }
    }
  ...
  filtroEventosMeta_2_2() {
    let dataMeta_1meses_2: any = JSON.parse(
      localStorage.getItem('dataMeta_1meses_2')!
    );
    ...
    if (this.selectEventos12_2)
      if (this.selectEventos12_2.length > 0) {
        ...
        let filtroCoronaMeta12_2 = selectEventos12_2.map(
          (div: any) => div.label
        );

        dataMeta_1meses_2.datasets = dataMeta_1meses_2.datasets.filter(
          (data: any, index: any) =>
            filtroCoronaMeta12_2.includes(filtrosListCoronaMeta12_2[index])
        );
      }
    ...
    if (this.plantasList11 || flagCoronaTotal)
      if (this.plantasList11.length > 0 || flagCoronaTotal) {
        ...
        let reportesAt: any[] = JSON.parse(
          ...
          (at: any) =>
            new Date(at.fechaReporte).getFullYear() === this.selectedAnioMeta_2
        );
        ...
        if (!flagCoronaTotal)
          if (this.selectedDivisionResumen11)
            if (this.selectPais11 != 'Corona Total')
              reportesAt = reportesAt.filter(
                (at) => at.padreNombre == this.selectedDivisionResumen11
              );
        if (flagCoronaTotal)
          if (this.selectedDivisionResumen11)
        ...
        let plantas = listaPlantas
          ...
          .sort();
        plantas.push(
          flagCoronaTotal ? 'Corona total' : this.selectedDivisionResumen11
        );
        ...
        filterQueryMeta.filterList = [
          {
            criteria: Criteria.EQUALS,
            field: 'anio',
            value1: this.selectedAnioMeta_2.toString(),
          },
          {
            criteria: Criteria.EQUALS,
            field: 'empresaId',
        ...
        if (this.selectPais11)
          filterQueryMeta.filterList.push({
            criteria: Criteria.EQUALS,
            field: 'pais',
            value1: this.selectPais11.toString(),
          });
        if (this.selectedDivisionResumen11)
          if (this.selectPais11 != 'Corona Total')
            filterQueryMeta.filterList.push({
              criteria: Criteria.EQUALS,
              field: 'nombreDivision',
              value1: this.selectedDivisionResumen11,
            });
          ...
          if (this.selectedDivisionResumen11)
            if (this.selectPais11 != 'Corona Total')
              filterQuery.filterList.push({
                ...
                value1: this.divisionesCoronaConId
                  .find(
                    (div: any) => div.nombre === this.selectedDivisionResumen11
                  )
                  .id.toString(),
              });
          ...
          await this.hhtService
            ...
            .then(async (res: any) => {
                ...
                if (this.selectPais11 != 'Corona Total')
                  filterQuery2.filterList.push({
                    ...
                    value1: this.divisionesCoronaConId
                      .find(
                        (div: any) =>
                          div.nombre === this.selectedDivisionResumen11
                      )
                      .id.toString(),
                  });
              ...
              if (res.data.length > 0 || hhtTemp!) {
                ...
                let totalDiasPerdidos = 0;
                let AtMortalesTotal = 0;
                let planta = flagCoronaTotal
                  ? this.divisionList11.filter((item: any) =>
                      plantas.includes(item.label)
                    )
                  : this.plantasList11.filter((item: any) =>
                      plantas.includes(item.label)
                    );
                ...
                planta.forEach((plantas: any) => {
                  ...
                  meta1 = flagCoronaTotal
                    ? metaTotal.find(
                        (met: any) => met.nombreDivision == plantaL
                      )
                    : meta.find((met: any) => met.nombrePlanta == plantaL);
                  ...
                  if (mesesFiltrados > 0)
                    ...
                    trabajadoresTotales =
                      trabajadoresTotales /
                      (this.selectedAnioMeta_2 == new Date().getFullYear()
                        ? new Date().getMonth() + 1
                        : 12);
                  ...
                  if (this.filtroMesesMeta_2.length > 0) {
                    ...
                    totalTrabajadoresTempMes += totalTrabajadoresTemp;
                    totalTrabajadoresTemp =
                      totalTrabajadoresTemp / this.filtroMesesMeta_2.length;
                  } else {
                    totalTrabajadoresTemp = trabajadoresPorMes.reduce(
                      (count: any, trabajadores: any) => {
                        return count + trabajadores;
                  ...
                  dataMetaTF.push(
                    meta1
                      ? meta1[flagCoronaTotal ? 'tfDivision' : 'tfPlanta']
                        ? meta1[flagCoronaTotal ? 'tfDivision' : 'tfPlanta']
                        : 0
                      : 0
                  );
                  dataMetaTS.push(
                    meta1
                      ? meta1[flagCoronaTotal ? 'tsDivision' : 'tsPlanta']
                        ? meta1[flagCoronaTotal ? 'tsDivision' : 'tsPlanta']
                        : 0
                      : 0
                  );
                ...
                let mesesYear =
                  this.selectedAnioMeta_2 == new Date().getFullYear()
                    ...
                    : 12;
                let numMesesSelect =
                  this.filtroMesesMeta_2.length > 0
                    ? this.filtroMesesMeta_2.length
                    : mesesYear;
                let totalesTrabajadores =
                  trabajadoresTotalesMes + totalTrabajadoresTempMes;
                ...
                dataMetaTF.push(
                  meta1
                    ? meta1[flagCoronaTotal ? 'tfAnual' : 'tfDivision']
                      ? meta1[flagCoronaTotal ? 'tfAnual' : 'tfDivision']
                      : 0
                    : 0
                );
                dataMetaTS.push(
                  meta1
                    ? meta1[flagCoronaTotal ? 'tsAnual' : 'tsDivision']
                      ? meta1[flagCoronaTotal ? 'tsAnual' : 'tsDivision']
                      : 0
                    : 0
                );
  ...
  filtroMeta_2() {
    ...
    if (this.selectPais11 != 'Corona Total')
      if (this.PlantaSelect11)
        if (this.PlantaSelect11.length > 0) {
          let plantasCoronaMeta11 = this.plantasList11.map(
            (div: any) => div.label
          );
          ...
          dataMeta_2.datasets[3].data = dataMeta_2.datasets[3].data.filter(
            (data: any, index: any) =>
              PlantaSelect.includes(plantasCoronaMeta11[index])
          );
          dataMeta_2.datasets[2].data = dataMeta_2.datasets[2].data.filter(
            (data: any, index: any) =>
              PlantaSelect.includes(plantasCoronaMeta11[index])
          );
          dataMeta_2.datasets[1].data = dataMeta_2.datasets[1].data.filter(
            (data: any, index: any) =>
              PlantaSelect.includes(plantasCoronaMeta11[index])
          );
          dataMeta_2.datasets[0].data = dataMeta_2.datasets[0].data.filter(
            (data: any, index: any) =>
              PlantaSelect.includes(plantasCoronaMeta11[index])
          );
        }
    ...
    if (this.selectPais11 == 'Corona Total')
      if (this.selectedDivisionResumen11)
        if (this.selectedDivisionResumen11.length > 0) {
          let plantasCoronaMeta11 = this.divisionList11.map(
            (div: any) => div.label
          );

          // let PlantaSelect:any=[]
          // PlantaSelect.push(this.selectedDivisionResumen11)
          let PlantaSelect = this.selectedDivisionResumen11
            .map((div: any) => div)
            .sort();
    ...
    if (this.selectEventos11)
      if (this.selectEventos11.length > 0) {
        ...
        dataMeta_2.datasets = dataMeta_2.datasets.filter(
          (data: any, index: any) =>
            filtroCoronaMeta11.includes(filtrosListCoronaMeta11[index])
        );
      }
    Object.assign(this, { dataMeta_2 });
  }
      ...
      if (this.plantasList11_2.length > 0 || flagCoronaTotal) {
        ...
        if (!flagCoronaTotal)
          if (this.selectedDivisionResumen11_2)
            if (this.selectPais11_2 != 'Corona Total')
              reportesAt = reportesAt.filter(
                (at) => at.padreNombre == this.selectedDivisionResumen11_2
              );
        if (flagCoronaTotal)
          if (this.selectedDivisionResumen11_2)
        ...
        let plantas = listaPlantas
          ...
          .sort();
        plantas.push(
          flagCoronaTotal ? 'Corona total' : this.selectedDivisionResumen11_2
        );
        ...
        filterQueryMeta.filterList = [
          {
            criteria: Criteria.EQUALS,
            field: 'anio',
            value1: this.selectedAnioMeta_2_2.toString(),
          },
          {
            criteria: Criteria.EQUALS,
            field: 'empresaId',
        ...
        if (this.selectPais11_2)
          filterQueryMeta.filterList.push({
            criteria: Criteria.EQUALS,
            field: 'pais',
            value1: this.selectPais11_2.toString(),
          });
        if (this.selectedDivisionResumen11_2)
          if (this.selectPais11_2 != 'Corona Total')
            filterQueryMeta.filterList.push({
              criteria: Criteria.EQUALS,
              field: 'nombreDivision',
              value1: this.selectedDivisionResumen11_2,
            });
            ...
            if (this.selectPais11_2 != 'Corona Total')
              filterQuery.filterList.push({
                ...
                value1: this.divisionesCoronaConId
                  .find(
                    (div: any) =>
                      div.nombre === this.selectedDivisionResumen11_2
                  )
                  .id.toString(),
              });
          ...
          await this.hhtService
            ...
            .then(async (res: any) => {
              ...
              if (res.data.length > 0 || hhtTemp!) {
                ...
                let planta = flagCoronaTotal
                  ? this.divisionList11_2.filter((item: any) =>
                      plantas.includes(item.label)
                    )
                  : this.plantasList11_2.filter((item: any) =>
                      plantas.includes(item.label)
                    );
                ...
                planta.forEach((plantas: any) => {
                  ...
                  if (mesesFiltrados > 0)
                    ...
                    trabajadoresTotales =
                      trabajadoresTotales /
                      (this.selectedAnioMeta_2_2 == new Date().getFullYear()
                        ? new Date().getMonth() + 1
                        : 12);
                  ...
                  if (this.filtroMesesMeta_2_2.length > 0) {
                    ...
                    totalTrabajadoresTempMes += totalTrabajadoresTemp;
                    totalTrabajadoresTemp =
                      totalTrabajadoresTemp / this.filtroMesesMeta_2_2.length;
                  } else {
                    totalTrabajadoresTemp = trabajadoresPorMes.reduce(
                      (count: any, trabajadores: any) => {
                        return count + trabajadores;
                ...
                let mesesYear =
                  this.selectedAnioMeta_2_2 == new Date().getFullYear()
                    ...
                    : 12;
                let numMesesSelect =
                  this.filtroMesesMeta_2_2.length > 0
                    ? this.filtroMesesMeta_2_2.length
                    : mesesYear;
                let totalesTrabajadores =
                  trabajadoresTotalesMes + totalTrabajadoresTempMes;
  ...
  filtroMeta_2_2() {
    ...
    if (this.selectPais11_2 != 'Corona Total')
      if (this.PlantaSelect11_2)
        if (this.PlantaSelect11_2.length > 0) {
          let plantasCoronaMeta11 = this.plantasList11_2.map(
            (div: any) => div.label
          );

          let PlantaSelect = this.PlantaSelect11_2.map(
            (div: any) => div
          ).sort();

          dataMeta_2_2.labels = PlantaSelect;
          dataMeta_2_2.datasets[3].data = dataMeta_2_2.datasets[3].data.filter(
            (data: any, index: any) =>
              PlantaSelect.includes(plantasCoronaMeta11[index])
          );
          dataMeta_2_2.datasets[2].data = dataMeta_2_2.datasets[2].data.filter(
            (data: any, index: any) =>
              PlantaSelect.includes(plantasCoronaMeta11[index])
          );
          dataMeta_2_2.datasets[1].data = dataMeta_2_2.datasets[1].data.filter(
            (data: any, index: any) =>
              PlantaSelect.includes(plantasCoronaMeta11[index])
          );
          dataMeta_2_2.datasets[0].data = dataMeta_2_2.datasets[0].data.filter(
            (data: any, index: any) =>
              PlantaSelect.includes(plantasCoronaMeta11[index])
          );
        }
    ...
    if (this.selectPais11_2 == 'Corona Total')
      if (this.selectedDivisionResumen11_2)
        if (this.selectedDivisionResumen11_2.length > 0) {
          let plantasCoronaMeta11 = this.divisionList11_2.map(
            (div: any) => div.label
          );

          // let PlantaSelect:any=[]
          // PlantaSelect.push(this.selectedDivisionResumen11)
          let PlantaSelect = this.selectedDivisionResumen11_2
            .map((div: any) => div)
            .sort();
    ...
    if (this.selectEventos11_2)
      if (this.selectEventos11_2.length > 0) {
        ...
        dataMeta_2_2.datasets = dataMeta_2_2.datasets.filter(
          (data: any, index: any) =>
            filtroCoronaMeta11.includes(filtrosListCoronaMeta11[index])
        );
      }
  ...
  async getTasas_2Meses(filter?: string) {
    if (this.selectPais13) {
      let reportesAt: any[] = JSON.parse(
        ...
        (at: any) =>
          new Date(at.fechaReporte).getFullYear() === this.filtroAnioTasa_2meses
      );
      ...
      if (this.selectPais13)
        if (this.selectPais13 != 'Corona Total')
          reportesAt = reportesAt.filter((at) => at.pais == this.selectPais13);
      if (this.selectedDivisionResumen13)
        reportesAt = reportesAt.filter(
          (at) => at.padreNombre == this.selectedDivisionResumen13
        );
        ...
        filterQuery.filterList = [
          {
            criteria: Criteria.EQUALS,
            field: 'anio',
            value1: this.filtroAnioTasa_2meses.toString(),
          },
          {
            criteria: Criteria.EQUALS,
            field: 'empresaSelect',
        ...
        if (this.selectPais13)
          if (this.selectPais13 != 'Corona Total')
            filterQuery.filterList.push({
              criteria: Criteria.EQUALS,
              field: 'planta.pais',
              value1: this.selectPais13.toString(),
            });
        if (this.selectedDivisionResumen13)
          filterQuery.filterList.push({
            criteria: Criteria.EQUALS,
            field: 'planta.area.nombre',
            value1: this.selectedDivisionResumen13,
          });
        // if(this.PlantaSelect5)filterQuery.filterList.push({criteria: Criteria.EQUALS, field: "planta.nombre", value1: this.PlantaSelect5.toString()})
        await this.hhtService
          .findByFilter(filterQuery)
          .then(async (res: any) => {
            ...
            let meses: any[] = [];
            if (this.filtroMesesMeta_2meses.length > 0)
              meses = this.MesesValue.filter((data) =>
                this.filtroMesesMeta_2meses.includes(data.label)
              );
            else meses = this.MesesValue.map((ele) => ele);
            ...
            if (res.data.length > 0 || hhtTemp!) {
              ...
              let meta1: any = meta.find(
                (met: any) =>
                  met.nombreDivision == this.selectedDivisionResumen13
              );
              meses.forEach((mes, index) => {
                let trabajadoresTotales2 = 0;
                let totalAt = 0;
                ...
                hhtTemp.forEach((hht, index) => {
                  ...
                  if (this.PlantaSelect13) {
                    if (this.PlantaSelect13.length > 0) {
                      this.PlantaSelect13.forEach((pl: any) => {
                        if (hht.mes === mes.label) {
                          if (pl.nombre == hht.planta_nombre)
                            trabajadoresTemPorArea += hht.numeroPersonas
                              ? hht.numeroPersonas
                              : 0;
                        }
                ...
                totalAt = reportesAt.filter(
                  (at) => mes.value === new Date(at.fechaReporte).getMonth()
                ).length;

                diasPerdidos = reportesAt
                  .filter(
                    (at) =>
                      mes.value === new Date(at.fechaReporte).getMonth() &&
                      at.incapacidades !== null &&
                      at.incapacidades !== 'null'
                  )
                  .reduce((count, item) => {
                ...
                dataMetaTF.push(
                  meta1 ? (meta1.tfDivision ? meta1.tfDivision : 0) : 0
                );
                dataMetaTS.push(
                  meta1 ? (meta1.tsDivision ? meta1.tsDivision : 0) : 0
                );
              });
              ...
              localStorage.setItem(
                'dataMeta_2meses',
                JSON.stringify(dataMeta_2meses)
              );
  ...
  filtroTasas_2meses() {
    let dataMeta_2meses: any = JSON.parse(
      localStorage.getItem('dataMeta_2meses')!
    );
    ...
    if (this.selectEventos13)
      if (this.selectEventos13.length > 0) {
        ...
        dataMeta_2meses.datasets = dataMeta_2meses.datasets.filter(
          (data: any, index: any) =>
            filtroCoronaMeta13.includes(filtrosListCoronaMeta13[index])
        );
      }
    Object.assign(this, { dataMeta_2meses });
  }
  ...
  async getTasas_2Meses_2(filter?: string) {
    if (this.selectPais13_2) {
      ...
      if (this.selectedDivisionResumen13_2)
        reportesAt = reportesAt.filter(
          (at) => at.padreNombre == this.selectedDivisionResumen13_2
        );
        ...
        filterQuery.filterList = [
          {
            criteria: Criteria.EQUALS,
            field: 'anio',
            value1: this.filtroAnioTasa_2meses_2.toString(),
          },
          {
            criteria: Criteria.EQUALS,
            field: 'empresaSelect',
        ...
        if (this.selectPais13_2)
          if (this.selectPais13_2 != 'Corona Total')
            filterQuery.filterList.push({
              criteria: Criteria.EQUALS,
              field: 'planta.pais',
              value1: this.selectPais13_2.toString(),
            });
        if (this.selectedDivisionResumen13_2)
          filterQuery.filterList.push({
            criteria: Criteria.EQUALS,
            field: 'planta.area.nombre',
            value1: this.selectedDivisionResumen13_2,
          });
        // if(this.PlantaSelect5)filterQuery.filterList.push({criteria: Criteria.EQUALS, field: "planta.nombre", value1: this.PlantaSelect5.toString()})
        await this.hhtService
          .findByFilter(filterQuery)
          .then(async (res: any) => {
            ...
            let meses: any[] = [];
            if (this.filtroMesesMeta_2meses_2.length > 0)
              meses = this.MesesValue.filter((data) =>
                this.filtroMesesMeta_2meses_2.includes(data.label)
              );
            else meses = this.MesesValue.map((ele) => ele);
            ...
            if (res.data.length > 0 || hhtTemp!) {
              ...
              let meta1: any = meta.find(
                (met: any) =>
                  met.nombreDivision == this.selectedDivisionResumen13_2
              );
              meses.forEach((mes, index) => {
                let trabajadoresTotales2 = 0;
                let totalAt = 0;
              ...
              localStorage.setItem(
                'dataMeta_2meses_2',
                JSON.stringify(dataMeta_2meses_2)
              );
  ...
  filtroTasas_2meses_2() {
    let dataMeta_2meses_2: any = JSON.parse(
      localStorage.getItem('dataMeta_2meses_2')!
    );
    let filtrosListCoronaMeta13_2 = this.grafMetas2.map(
      (div: any) => div.label
    );
    if (this.selectEventos13_2)
      if (this.selectEventos13_2.length > 0) {
        let selectEventos13_2: any = this.order(this.selectEventos13_2);
        let filtroCoronaMeta13_2 = selectEventos13_2.map(
          (div: any) => div.label
        );

        dataMeta_2meses_2.datasets = dataMeta_2meses_2.datasets.filter(
          (data: any, index: any) =>
            filtroCoronaMeta13_2.includes(filtrosListCoronaMeta13_2[index])
        );
      }
    Object.assign(this, { dataMeta_2meses_2 });
  }
  ...
  contTotal(datos: any) {
    ...
    datos.forEach((resp: any) => {
      resp.series.forEach((resp2: any) => {
        if (total.has(resp2.name)) {
          total.set(resp2.name, total.get(resp2.name) + resp2.value);
        } else {
          total.set(resp2.name, resp2.value);
  ...
  async getPlantas(pais: string | null) {
    if (pais) {
      ...
      if (pais != 'Corona Total')
        filterPlantaQuery.filterList.push({
          field: 'pais',
          criteria: Criteria.EQUALS,
          value1: pais,
        });
      ...
      return new Promise(async (resolve, reject) => {
        await this.plantasService
          .getPlantaWithFilter(filterPlantaQuery)
          .then((res: any) => {
            this.plantasList = (<Plantas[]>res.data).map((planta) => planta);
            for (const ele of this.divisionList) {
              if (this.getPlantasByArea(ele.id, res.data))
  ...
  funcSelectDivision(div: any, filter: any) {
    if (div.value) {
      ...
      plantasList = this.plantasList.filter(
        (pl1: any) => pl1.area_id == dv[0].id
      );
      // }
      // else{
      ...
      switch (filter) {
        ...
        case 'tasa1':
          ...
          this.plantasList4 = [];
          for (const pl of plantasList) {
            this.plantasList4.push({
              label: pl.nombre,
              value: { nombre: pl.nombre, id: pl.id },
            });
          }
          this.getTasas_1('filtro');
          break;
