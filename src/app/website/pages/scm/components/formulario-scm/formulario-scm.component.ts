...
import {
  locale_es,
  tipo_identificacion,
} from '../../../comun/entities/reporte-enumeraciones';
...
export class FormularioScmComponent implements OnInit, OnDestroy {
  ...
  empresaList2: empresaNit[] = [
    ...
    {
      label: 'Almacenes Corona',
      empresa: 'Almacenes Corona',
      nit: '860500480-8',
    },
    {
      label: 'Compañía Colombiana de Ceramica S.A.S',
      empresa: 'Compañía Colombiana de Ceramica S.A.S',
      nit: '860002536-5',
    },
    { label: 'Corlanc S.A.S', empresa: 'Corlanc S.A.S', nit: '900481586-1' },
    {
      label: 'Corona Industrial',
      empresa: 'Corona Industrial',
      nit: '900696296-4',
    },
    {
      label: 'Despachadora internacional de Colombia S.A.S',
      empresa: 'Despachadora internacional de Colombia S.A.S',
      nit: '860068121-6',
    },
    {
      label: 'Electroporcelana Gamma',
      empresa: 'Electroporcelana Gamma',
      nit: '890900121-4',
    },
    {
      label: 'Locería Colombiana S.A.S',
      empresa: 'Locería Colombiana S.A.S',
      nit: '890900085-7',
    },
    {
      label: 'Minerales Industriales S.A',
      empresa: 'Minerales Industriales S.A',
      nit: '890917398-1',
    },
    { label: 'Nexentia S.A.S', empresa: 'Nexentia S.A.S', nit: '900596618-3' },
    {
      label: 'Suministros de Colombia S.A.S',
      empresa: 'Suministros de Colombia S.A.S',
      nit: '890900120-7',
    },
    {
      label: 'Organización corona',
      empresa: 'Organización corona',
      nit: '860002688-6',
    },
    {
      label: 'Industria Ceramica Costarricense',
      empresa: 'Industria Ceramica Costarricense',
      nit: '1-811-0957',
    }, //honduras - costarica
    {
      label: 'Industria Centroamericana de Sanitarios',
      empresa: 'Industria Centro Americana de Sanitarios',
      nit: '1078',
    }, //honduras - costarica
    { label: 'Sociedad Anonima', empresa: 'Sociedad Anonima', nit: '7568991' }, // Guatemala
    {
      label: 'Porcelana Corona de México, S.A. de C.V.',
      empresa: 'Porcelana Corona de México, S.A. de C.V.',
      nit: 'SLA630306CF7',
    }, //Moterrey - Ramos Arispe
    {
      label: 'Industria Cerámica Centroamericana, S.A.',
      empresa: 'Industria Cerámica Centroamericana, S.A.',
      nit: 'J0310000003211',
    }, //Nicaragua
  ];
  ...
  tienePermisocambiarestado: boolean = false;
  constructor(
    ...
    this.status = this.caseStatus.find(
      (sta) => sta.value == this.casoMedicoForm.get('statusCaso')?.value
    )?.label;
  ...
  get disableRazon() {
    if (
      this.casoMedicoForm.value.casoMedicoLaboral === 2 ||
      this.casoMedicoForm.value.casoMedicoLaboral === 3
    ) {
      return true;
    }
    return false;
  ...
  async ngOnInit() {
    let primerNombre = this.sesionService.getEmpleado()
      ? this.sesionService.getEmpleado()!.primerNombre
        ? this.sesionService.getEmpleado()!.primerNombre
        : ''
      : '';
    let segundoNombre = this.sesionService.getEmpleado()
      ? this.sesionService.getEmpleado()!.segundoNombre
        ? this.sesionService.getEmpleado()!.segundoNombre
        : ''
      : '';
    let primerApellido = this.sesionService.getEmpleado()
      ? this.sesionService.getEmpleado()!.primerApellido
        ? this.sesionService.getEmpleado()!.primerApellido
        : ''
      : '';
    let segundoApellido = this.sesionService.getEmpleado()
      ? this.sesionService.getEmpleado()!.segundoApellido
        ? this.sesionService.getEmpleado()!.segundoApellido
        : ''
      : '';
    this.nombreSesion =
    ...
    this.consultar =
      localStorage.getItem('scmShowCase') === 'true' ? true : false;
    this.consultar2 =
      localStorage.getItem('scmShowCase') === 'true' ? true : false;
    this.tienePermisocambiarestado =
      this.sesionService.getPermisosMap()['SCM_PUT_CAMBIAR_ESTADO'];
    console.log(this.tienePermisocambiarestado);

    try {
      let res: any = await this.scmService.getSvelist();
      this.sveOptionList.push({ label: '--Seleccione--', value: null });
      res.forEach((sve: any) => {
        this.sveOptionList.push({
          label: sve.nombre,
          value: sve.id.toString(),
        });
      });
    ...
    await this.comunService.findAllPrepagadas().then((data) => {
      ...
      (<Prepagadas[]>data).forEach((prepagadas) => {
        this.prepagadasList.push({
          label: prepagadas.nombre,
          value: prepagadas.id,
        });
      });
    ...
    await this.comunService.findAllJuntas().then((data) => {
      ...
      (<JuntaRegional[]>data).forEach((JuntaRegional) => {
        this.JuntaRegionalList.push({
          label: JuntaRegional.nombre,
          value: JuntaRegional.id,
        });
      });
  ...
  async aonRegisters() {
    try {
      ...
      let res: any = await this.scmService.getRegistersAon(
  ...
  async onSubmit() {
    ...
    try {
      ...
      if (region === 'SELECCIONE UBICACION') {
        this.messageService.add({
          ...
          severity: 'error',
          detail:
            'Debe actualizar la ubicación del trabajador involucrado en la pestaña Información General',
          life: 6000,
        });
        this.flagGuardado = false;
        return;
      ...
      this.messageService.add({
        ...
        severity: 'error',
        detail:
          'Por favor revise los campos ciudad de residencia en la pestaña información general.',
        life: 6000,
      });
      this.flagGuardado = false;
    ...
    this.casoMedicoForm.patchValue({
      ...
      eliminado: false,
      fechaFinal:
        this.casoMedicoForm.controls['fechaFinal'].value == null
          ? null
          : this.casoMedicoForm.controls['fechaFinal'].value,
  ...
  async buscarEmpleado2(event: any) {
    ...
    for (let i = 1; i < this.fields.length; i++) {
      ...
      if (this.fields[i] != 'usuarioBasic') {
        ...
        filterQuery.filterList.push({
          criteria: Criteria.LIKE,
          field: 'usuarioBasic.email',
          value1: '%' + event.query + '%',
        });
      }

      let terminarBusqueda = false;
      await this.empleadoService.findByFilter(filterQuery).then((data: any) => {
  ...
  async onSelection(event: any) {
    ...
    if (this.empleadoSelect) {
      this.empresaForm!.value.nit = this.empleadoSelect.nit;
      this.empresaForm!.value.empresa = {
        label: this.empleadoSelect.empresa,
  ...
  async submitEmp() {
    ...
    empleado.phoneEmergencyContact =
      this.empleadoForm.value.phoneEmergencyContact;
    empleado.emailEmergencyContact =
      this.empleadoForm.value.emailEmergencyContact;
    ...
    empleado.numeroIdentificacion =
      this.empleadoForm.value.numeroIdentificacion;
    empleado.telefono1 = this.empleadoForm.value.telefono1;
    empleado.telefono2 = this.empleadoForm.value.telefono2;
    empleado.ciudad =
      this.empleadoForm.value.ciudad == null
        ? null
        : this.empleadoForm.value.ciudad.id;
    ...
    empleado.empresa =
      this.empresaForm!.value.empresa == null
        ? null
        : this.empresaForm!.value.empresa.label;
    empleado.nit =
      this.empresaForm!.value.empresa == null
        ? 0
        : this.empresaForm!.value.empresa.nit;
  ...
  async onCloseModalrecomendation() {
    if (this.sesionService.getPermisosMap()['SCM_LIST_CASE_RECO'])
      this.recomendationList = await this.scmService.getRecomendations(
        this.caseSelect.id
      );
    this.modalRecomendatios = false;
    this.recoSelect = null;
    if (this.sesionService.getPermisosMap()['SCM_GET_CASE_LOG'])
      this.logsList = await this.scmService.getLogs(this.caseSelect.id);
  }

  async onCloseModalseguimiento() {
    if (this.sesionService.getPermisosMap()['SCM_GET_CASE_SEG'])
      this.seguimientosList = await this.scmService.getSeguimientos(
        this.caseSelect.id
      );
    if (this.sesionService.getPermisosMap()['SCM_GET_CASE_SEG'])
      this.seguimientos = await this.scmService.getSeguimientos(
        this.caseSelect.id
      );
  ...
  async onCloseModalseguimientogenerico() {
    if (this.sesionService.getPermisosMap()['SCM_GET_CASE_SEG_GENERICO'])
      this.seguimientosgenericoList =
        await this.scmService.getSeguimientosgenerico(this.caseSelect.id);
    if (this.sesionService.getPermisosMap()['SCM_GET_CASE_SEG_GENERICO'])
      this.seguimientosgenerico = await this.scmService.getSeguimientosgenerico(
        this.caseSelect.id
      );
  ...
  async copiarLinkSeguimiento(
    ...
    await this.firmaservice
      ...
      .then(async (ele: any) => {
        ...
        if (
          new Date(
            new Date(firm.fechacreacion)!.getTime() + 1000 * 60 * 60 * 24
          ) < new Date()
        ) {
          if (firm.fecharenovacion) {
            if (
              new Date(
                new Date(firm.fecharenovacion)!.getTime() + 1000 * 60 * 60 * 24
              ) < new Date()
            ) {
    ...
    if (firmaExiste) {
      ...
      this.messageService.add({
        ...
        key: 'formScm',
        severity: 'info',
        summary: 'Link firmado',
        detail: 'Este link ya se encuentra con una firma registrada',
      });
    }
  ...
  async onCloseModalDianostico() {
    if (this.sesionService.getPermisosMap()['SCM_GET_CASE_DIAG'])
      this.diagnosticoList = await this.scmService.getDiagnosticos(
        this.caseSelect.id
      );
    this.modalDianostico = false;
    this.diagSelect = null;
  ...
  async modifyCase() {
    ...
    if (this.caseSelect) {
      let {
        fechaCalificacion,
        emisionPclFecha,
        fechaConceptRehabilitacion,
        fechaConceptRehabilitacionTwo,
        fechaFinal,
        ...caseFiltered
      } = this.caseSelect;
      ...
      this.casoMedicoForm.patchValue({
        fechaConceptRehabilitacion:
          fechaConceptRehabilitacion == null
            ? null
            : new Date(fechaConceptRehabilitacion),
        fechaConceptRehabilitacionTwo:
          fechaConceptRehabilitacionTwo == null
            ? null
            : new Date(fechaConceptRehabilitacionTwo),
        fechaCalificacion:
          fechaCalificacion == null ? null : new Date(fechaCalificacion),
        emisionPclFecha:
          emisionPclFecha == null ? null : new Date(emisionPclFecha),
      ...
      try {
        ...
        this.fechaSeg();
        if (this.sesionService.getPermisosMap()['SCM_GET_CASE_TRAT'])
          this.tratamientos = await this.scmService.getTratamientos(
            this.caseSelect.id
          );
        this.empresaId = this.sesionService.getEmpresa()?.id;
        this.aonRegisters();
        this.status =
          this.casoMedicoForm.get('statusCaso')?.value !== null
            ? this.caseStatus.find(
                (sta) =>
                  sta.value == this.casoMedicoForm.get('statusCaso')?.value
              )?.label
            : 'N/A';
        if (this.sesionService.getPermisosMap()['SCM_GET_CASE_LOG'])
          this.logsList = await this.scmService.getLogs(this.caseSelect.id);
  ...
  async onRowCloneInit(pseg: any, type?: any) {
    let { id, tarea, responsable, resultado, responsableExterno, ...product } =
      pseg;
    try {
      let firm = new firma();
  ...
  async anexo6seguimiento(seguimiento: any) {
    ...
    filterQuery.filterList.push({
      criteria: Criteria.EQUALS,
      field: 'idrelacionado',
      value1: seguimiento.id.toString(),
    });
    ...
    template
      ...
      ?.setAttribute('src', this.sesionService.getEmpresa()?.logo!);
    template
      ?.querySelector('#P_firma_aprueba')
      ?.setAttribute('src', '../../../../../assets/png/imgwhite.png');
    if (ele.length > 0)
      if (ele[0].firma)
        template
          ?.querySelector('#P_firma_aprueba')
          ?.setAttribute('src', ele[0].firma);
    template
      ?.querySelector('#P_firma_medico')
      ?.setAttribute('src', '../../../../../assets/png/imgwhite.png');
    if (ele.length > 0)
      if (ele[1].firma)
        template
          ?.querySelector('#P_firma_medico')
          ?.setAttribute('src', ele[1].firma);
  ...
  cambiarEstado(event?: any) {
    ...
    if (event && event == 'cerrar') {
      if (
        this.casoMedicoForm.controls['fechaFinal'].value != null &&
        this.casoMedicoForm.controls['fechaFinal'].value != '' &&
        this.casoMedicoForm.controls['observaciones'].value != null &&
        this.casoMedicoForm.controls['observaciones'].value != ''
      ) {
        this.confirmationService.confirm({
          ...
          accept: () => {
            this.scmService.edit(this.casoMedicoForm.value).then(() => {
              this.scmService
                .changeEstadoById(this.caseSelect.id)
                .then((res) => {
                  this.msgs = [];
                  this.messageService.add({
                    // this.msgs.push({
        ...
        this.messageService.add({
          ...
          summary: 'Error',
          detail:
            'Debe completar la información: Fecha de cierre y Observaciones del cierre',
        });
      }
    } else {
      this.confirmationService.confirm({
  ...
  permisoFechaCierre(): boolean {
    let tienePermiso =
      this.sesionService.getPermisosMap()['SCM_PUT_CAMBIAR_FECHAFIN'];
