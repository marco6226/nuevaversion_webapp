...
export class FormularioAccidenteComponent implements OnInit, AfterViewInit {
  ...
  constructor(
    ...
    this.tipoIdentificacion = defaultItem.concat(
      <SelectItem[]>tipo_identificacion
    );
    this.tipoIdentificacionEmpresa = defaultItem.concat(
      <SelectItem[]>tipo_identificacion_empresa
    );
    ...
    this.tienePermisocamex =
      this.sesionService.getPermisosMap()['ADM_GET_CAMEX'];
  }
  ...
  cargarDatos() {
    ...
    if (this.reporte)
      if (
        this.reporte.fechaIngresoEmpleado != null &&
        this.reporte.fechaAccidente != null
      ) {
        this.fechaIngreso = new Date(this.reporte.fechaIngresoEmpleado);
        this.fechaAccidente = new Date(this.reporte.fechaAccidente);
        this.deltaFecha = new Date(
          this.fechaAccidente.getTime() - this.fechaIngreso.getTime()
        );
        this.reporte.mesesLaborHabitual =
          this.deltaFecha.getMonth() +
          12 * (this.deltaFecha.getFullYear() - 1970);
        this.reporte.diasLaborHabitual =
          this.deltaFecha.getDate() == 31 ? 0 : this.deltaFecha.getDate();
    ...
    this.form = this.fb.group({
      ...
      fechaNacimientoEmpleado:
        this.reporte?.fechaNacimientoEmpleado == null
          ? null
          : new Date(this.reporte.fechaNacimientoEmpleado),
      ...
      fechaIngresoEmpleado:
        this.reporte?.fechaIngresoEmpleado == null
          ? null
          : new Date(this.reporte.fechaIngresoEmpleado),
      ...
      fechaAccidente:
        this.reporte?.fechaAccidente == null
          ? null
          : new Date(this.reporte.fechaAccidente),
      horaAccidente:
        this.reporte?.horaAccidente == null
          ? null
          : new Date(this.reporte.horaAccidente),
      ...
      horaInicioLabor:
        this.reporte?.horaInicioLabor == null
          ? null
          : new Date(this.reporte.horaInicioLabor),
      ...
      tipoIdentificacionResponsable:
        this.reporte?.tipoIdentificacionResponsable,
      numeroIdentificacionResponsable:
        this.reporte?.numeroIdentificacionResponsable,
      cargoResponsable: this.reporte?.cargoResponsable,
      fechaReporte:
        this.reporte?.fechaReporte == null
          ? null
          : new Date(this.reporte.fechaReporte),
    ...
    setTimeout(async () => {
      ...
      if (this.idEmpresa == '22' || this.idEmpresa == '508')
        await this.listadoLocalidades(
          this.form?.value.areaAccidente.padreNombre
        );
    }, 2000);
  ...
  onSubmit() {
    ...
    if (this.adicionar) {
      this.reporteService.create(reporte).then((data) => {
        this.onSave.emit(<Reporte>data);
      });
    } else if (this.modificar) {
      this.reporteService
        .update(reporte)
        .then((data) => this.onSave.emit(<Reporte>data));
    }
  }
  ...
  changeCalendar() {
    if (this.form && this.reporte)
      if (
        this.form.value.fechaIngresoEmpleado != null &&
        this.form.value.fechaAccidente != null
      ) {
  ...
  async listadoLocalidades(event: any) {
    ...
    filterLocalidad.fieldList = ['id', 'localidad'];
    filterLocalidad.filterList = [
      {
    ...
    await this.empresaService
      .getLocalidadesRWithFilter(filterLocalidad)
      .then((ele: any) => {
