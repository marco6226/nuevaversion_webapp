...
export class GestionDocumentalComponent implements OnInit {
  ...
  nodeSelect?: TreeNode | null;
  nodoPadre?: TreeNode | null;
  loading?: boolean;
  ...
  recarguesimple(event :any, actual:number) {
  ...
  generarModelo(dirList: Directorio[], nodoPadre: TreeNode | null) {
      let model:any = [];
      dirList.forEach((dir) => {
  ...
  crearNuevaCarpeta() {
      ...
      let dir:any = new Directorio();
      dir.directorioPadre=null;
      if (this.nodeSelect != null) {
          // Determina el nodo padre de tipo carpeta
          let nodoPadre :any = this.nodeSelect.data.esDocumento ? this.nodeSelect.parent : this.nodeSelect;
  ...
  adicionarRegistroAArbol(dir: Directorio, nodo: TreeNode | null) {
      if (nodo != null) {
          let nodoPadre:any = nodo.data.esDocumento ? nodo.parent : nodo;
  ...
  onDoubleClick(event:any) {
      ...
      if (!nodeSelect.data.esDocumento) {
          this.consultarNodos(nodeSelect).then((data:any) => {
              ...
              parents.forEach((parent) => {
                  let item = {
                      ...
                      command: (event:any) => this.reconstruir(event),
                      nodo: parent,
                      indice: index--,
                  };
  ...
  reconstruir(event:any) {
      ...
      if (event.item.nodo == null) {
          ...
          this.directorioService.findByFilter(filterQuery).then((data:any) => {
              let dirList = this.inicializarFechas(<Directorio[]>data['data']);
  ...
  onNodeExpand(event: any) {
      let nodo = event.node;
      this.loading = true;
      this.consultarNodos(nodo).then((data:any) => {
          let children :any = [];
          let dirList = this.inicializarFechas(<Directorio[]>data['data']);
  ...
  dragStart(event:any, param: any) {
      this.draggedNode = param.node;
  }

  async drop(event:any, dropParam: any) {
