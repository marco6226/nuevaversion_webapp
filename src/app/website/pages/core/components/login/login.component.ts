...
export class LoginComponent implements OnInit, AfterViewInit {
  ...
  esAmbienteDemo(url: string): boolean {
    // Verifica si la URL contiene una cadena específica para el ambiente demo
    return url === 'https://demo.sigess.app/login';
    console.log(url);
  }

  ngOnInit(): void {
    const currentUrl = window.location.href;
    this.esDemo = this.esAmbienteDemo(currentUrl);

    console.log(this.esDemo);
    console.log(currentUrl);

    this.version = this.sesionService.getAppVersion();

    if (
      this.sesionService.getEmpresa() != null &&
      this.sesionService.getUsuario() != null
    ) {
  ...
  ngAfterViewInit(): void {
    if (localStorage.getItem('url'))
      this.messageService.add({
        severity: 'warn',
        summary: 'Advertencia',
        detail: 'Debe iniciar sesión para acceder a esta ruta.',
        life: 5000,
      });
  }
  ...
  async validate(value: any) {
    let res: any;
    try {
      res = await this.authService.checkisLoginExist(
        value.username,
        value.passwd
      );
      if (res.exit == 'true') {
        if (
          confirm('Se perderan los cambios no guardados de sus otras sesiones')
        ) {
          // Save it!
          this.login(value);
        }
  ...
  async login(value: any) {
    ...
    try {
      await this.authService
        .login(value.correo, value.password, value.recordar, value.pin)
        .then((res) => {
          let aceptaTerm =
            this.authService.sesionService.getUsuario()!.fechaAceptaTerminos !=
            null;
          if (aceptaTerm) {
      ...
      if (err['name'] != null && err['name'] == 'TimeoutError') {
        this.messageService.add({
          severity: 'warn',
          summary: 'CONEXIÓN DEFICIENTE',
          detail:
            'La conexión está tardando mucho tiempo en responder, la solicitud ha sido cancelada. Por favor intente mas tarde.',
        });
        return;
      }
      ...
      switch (err.status) {
        case 403:
          this.messageService.add({
            ...
            detail: 'El usuario o contraseña especificada no son correctas',
          });
          break;
        case 401:
          if (msg.codigo == 2_007) {
        ...
        default:
          this.messageService.add({
            severity: 'error',
            summary: 'ERROR',
            detail: 'Se ha generado un error no esperado',
          });
          break;
      }
  ...
  setVisible(visible: boolean) {
    this.messageService.add({
      severity: 'warn',
      summary: 'pro',
      detail: 'Se cerro su sesion inicie de nuevo por favor',
    });
