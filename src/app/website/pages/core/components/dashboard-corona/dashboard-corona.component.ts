...
export class DashboardCoronaComponent implements OnInit {
  ...
  originalText: string = 'Este es un texto que quiero comprimir.';
  ...
  Eventos: any[] = [
    { label: 'Numero AT', value: 0 },
    { label: 'Numero días perdidos', value: 1 },
    { label: 'Numero AT mortales', value: 2 },
    { label: 'Numero AT con cero días', value: 3 },
  ];
  ...
  constructor(
    ...
    private config: PrimeNGConfig // Ejemplo de compresión // this.compressText(); // this.decompressText();
  ) {}
  ...
  async dataSegundaGrafica() {
    ...
    this.date2 = new Date();
    await this.caracterizacionViewService
      .findAllCAR2()
      .then(async (resp: any) => {
  ...
  async CardsClasificacion() {
    ...
    if (this.date1 && !this.date2)
      this.CaracterizacionView1 = this.CaracterizacionView.filter(
        (resp: any) => {
    ...
    if (!this.date1 && this.date2) {
      let date2: Date = new Date(
        new Date(this.date2).setMonth(new Date(this.date2).getMonth() + 1)
      );
      this.CaracterizacionView1 = this.CaracterizacionView.filter(
        (resp: any) => {
    ...
    if (this.date1 && this.date2) {
      ...
      this.CaracterizacionView1 = this.CaracterizacionView.filter(
        (resp: any) => {
          return (
            resp.fechaaccidente < date2 &&
            resp.fechaaccidente >= new Date(this.date1!)
          );
        }
      );
    }
    ...
    if (this.selectPais1)
      if (this.selectPais1 != 'Corona Total')
        this.CaracterizacionView1 = this.CaracterizacionView1.filter(
          (at: any) => at.pais == this.selectPais2
        );
    if (this.selectedDivisionResumen2)
      if (this.selectedDivisionResumen2.length > 0) {
        reportesAtCopyDiv = [];
        this.selectedDivisionResumen2.forEach((element: any) => {
          reportesAtCopyDiv = reportesAtCopyDiv.concat(
            this.CaracterizacionView1.filter(
              (at: any) => at.padrenombre == element
            )
          );
        });
        this.CaracterizacionView1 = [...reportesAtCopyDiv];
      }
    if (this.LocalidadSelect1)
      if (this.LocalidadSelect1.length > 0) {
        reportesAtCopyDiv = [];
        this.LocalidadSelect1.forEach((element: any) => {
          reportesAtCopyDiv = reportesAtCopyDiv.concat(
            this.CaracterizacionView1.filter(
              (at: any) => at.nombreLocalidad == element
            )
          );
        });
        this.CaracterizacionView1 = [...reportesAtCopyDiv];
    ...
    this.CaracterizacionView1.forEach((element: any) => {
      if (this.radioGra0 == 0) {
        if (
          this.selectArea.length == 0 ||
          this.selectArea.toString() == 'Corona total'
        )
          this.ContCardsClasificacion(element);
        if (
          this.selectArea == element.padrenombre &&
          this.selectArea.toString() != 'Corona total'
        )
          this.ContCardsClasificacion(element);
      }
    ...
    if (filter != 'resumen2') {
      ...
      if (div.value.length > 0) {
        ...
        div.value.forEach((element: any) => {
          ...
          localidadesList = localidadesList.concat(
            this.localidadesList.filter(
              (loc: any) => loc.plantas_area_id == dv[0].id
            )
          );
        });
  ...
  tableroRiesgoIncial() {
    ...
    //let descomprimido=LZString.decompress(dataRiesgoInicial);
