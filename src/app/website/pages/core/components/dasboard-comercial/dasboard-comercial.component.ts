...
// @Component({
//   selector: 'app-dasboard-comercial',
//   standalone: true,
//   imports: [],
//   templateUrl: './dasboard-comercial.component.html',
//   styleUrl: './dasboard-comercial.component.scss'
// })

@Component({
  selector: 'app-dasboard-comercial',
  templateUrl: './dasboard-comercial.component.html',
  styleUrls: ['./dasboard-comercial.component.scss'],
  providers: [HhtService, SesionService,CaracterizacionViewService,ReporteAtService], 
})
export class DasboardComercialComponent {
  ili: number | any = 0;
  ...
  divisionesComercial: string[] = ['Almacenes Comercial', 'Bathrooms and Kitchen', 'Comercial Comercial Colombia', 'Funciones Transversales', 'Insumos Industriales y Energias', 'Mesa Servida', 'Superficies, materiales y pinturas','Comercial total'];
  divisionesComercialConId: any[] = [];
  reporteTabla?:any;
  reporteTabla2?:any;
    ...
    .then(
      (resp:any) => {
        ...
        this.areaList.forEach(element => {
          this.divisionesComercialConId.push({nombre: element.nombre, id: element.id});
  ...
  paisesList: Array<any> = [
    ...
    {label: 'Comercial Total', value: 'Comercial Total'}
  ];
  ...
  async getPlantas(pais:string | null) {
    if(pais){
      ...
      if(pais !='Comercial Total')filterPlantaQuery.filterList.push({ field: 'pais', criteria: Criteria.EQUALS, value1: pais })
  ...
  async loadResumen(){
    let filterQueryComercialmeta = new FilterQuery();
    let filterQueryComercial = new FilterQuery();
    ...
    filterQueryComercialmeta.sortOrder = SortOrder.ASC;
    filterQueryComercialmeta.sortField = "id";
    filterQueryComercialmeta.filterList = [
    ...
    if(this.selectPais1)if(this.selectPais1 != 'Comercial Total')filterQueryComercialmeta.filterList.push({criteria: Criteria.EQUALS, field: "pais", value1: this.selectPais1.toString()})
    if(this.selectedDivisionResumen1)filterQueryComercialmeta.filterList.push({criteria: Criteria.EQUALS, field: "nombreDivision", value1: this.selectedDivisionResumen1.toString()})
    if(this.PlantaSelect1)filterQueryComercialmeta.filterList.push({criteria: Criteria.EQUALS, field: "nombrePlanta", value1: this.PlantaSelect1.toString()})

    filterQueryComercial.sortOrder = SortOrder.ASC;
    filterQueryComercial.sortField = "id";
    filterQueryComercial.filterList = [
      {criteria: Criteria.EQUALS, field: "anio", value1: this.anioActualResumen.toString()},
      {criteria: Criteria.EQUALS, field: "empresa.id", value1: empresaId},
      {criteria: Criteria.EQUALS, field: "empresaSelect", value1: empresaId}
    ];
    filterQueryComercial.fieldList=this.fieldHht;

    if(this.selectPais1)if(this.selectPais1 != 'Comercial Total')filterQueryComercial.filterList.push({criteria: Criteria.EQUALS, field: "planta.pais", value1: this.selectPais1.toString()})
    if(this.selectedDivisionResumen1)filterQueryComercial.filterList.push({criteria: Criteria.EQUALS, field: "planta.area.id", value1: this.divisionesComercialConId.find((div:any) => div.nombre === this.selectedDivisionResumen1).id.toString()})
    if(this.PlantaSelect1)filterQueryComercial.filterList.push({criteria: Criteria.EQUALS, field: "planta.nombre", value1: this.PlantaSelect1.toString()})
    ...
    if(this.selectPais1)if(this.selectPais1 != 'Comercial Total')filterQueryTemp.filterList.push({criteria: Criteria.EQUALS, field: "planta.pais", value1: this.selectPais1.toString()})
    if(this.selectedDivisionResumen1)filterQueryTemp.filterList.push({criteria: Criteria.EQUALS, field: "planta.area.id", value1: this.divisionesComercialConId.find((div:any) => div.nombre === this.selectedDivisionResumen1).id.toString()})
...
//nuevo
  if(this.selectPais1)if(this.selectPais1!='Comercial Total')reportesAt = reportesAt.filter(at => at.pais == this.selectPais1);
...
//fin nuevo
    ...
    await this.viewHHtMetasService.getWithFilter(filterQueryComercialmeta).then(async (res: any) => {
    ...
    await this.hhtService.findByFilter(filterQueryComercial).then(async (res: any) => {
      if(res.data.length > 0){
  ...
  calcularTotalHht(hht: Hht[], mesInicio: number, mesFinal: number, selectPais:string, selectedDivisionResumen:string, PlantaSelect:string ): number{
    ...
    if(selectPais)if(selectPais != 'Comercial Total')hht=hht.filter((eve:any)=>eve.planta_pais==selectPais)
    if(selectedDivisionResumen)hht=hht.filter((eve:any)=>eve.planta_area_id==this.divisionesComercialConId.find((div:any) => div.nombre === selectedDivisionResumen).id)
  ...
  async getLocalidades(pais:string | null){
    if(pais){
      ...
      if(pais !='Comercial Total')filterLocalidadQuery.filterList.push({ field: 'plantas.pais', criteria: Criteria.EQUALS, value1: pais })
  ...
  async CardsClasificacion(){
    ...
    if(this.selectPais1)if(this.selectPais1!='Comercial Total')this.CaracterizacionView1 = this.CaracterizacionView1.filter((at:any) => at.pais == this.selectPais2);
    ...
    this.CaracterizacionView1.forEach((element:any) => {
      if(this.radioGra0==0){
        if(this.selectArea.length==0 || this.selectArea.toString()=='Comercial total')this.ContCardsClasificacion(element);
        if(this.selectArea==element.padrenombre  && this.selectArea.toString()!='Comercial total')this.ContCardsClasificacion(element);
      }
...
//Tercera grafica
  ...
  async cargarDatos(){
    ...
    // })
    // this.divisiones0.push({label:'Comercial total',value:'Comercial total'})
    // this.divisiones2.push('Comercial total')
  ...
  tableroRiesgoIncial(){
    ...
    this.resumenInicial['Comercial Total']=data
    this.resumenInicialText['Comercial Total']=dataT
  ...
  tableroRiesgofinal(){
    ...
    this.resumenResidual['Comercial Total']=data
    this.resumenResidualText['Comercial Total']=dataT
